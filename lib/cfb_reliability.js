...
(function (root, factory) {
  ...
  function teamSigma(side, b, fbs, P, cfg) {
  ...
  R.stability = function (p, o, over) {
    ...
    if (rating && rating.available) {
      [['home', bh, fbs.home], ['away', ba, fbs.away]].forEach(function (x) {
        var t = teamSigma(x[0], x[1], x[2], P, cfg);
  ...
  function rosterAvailability(c) {
    var items = [], cfg = c.cfg, unknownQb = [], contested = [], unconfirmed = [];
    ...
    ['home', 'away'].forEach(function (side) {
      ...
      if (who == null || who <= 0) {
        ...
        var confirmed = status === 'ANNOUNCED' || (st && st.confirmed === true);
        var why = confirmed ? null
          : (player ? player : 'the resolved starter') + (isComp ? ' is in an open competition'
        ...
        if (isComp) contested.push(name);
      }
      /* can he play */
      var avail = comp && comp.available ? num(comp.available.value) : null;
  ...
  function nonQb(c, side) {
    ...
    var comprehensive = !!(pers && pers.coverage && pers.coverage.comprehensive);
    var official = !!(pers && pers.coverage && pers.coverage.official);
    ...
    else if (st === 'FETCH_FAILED') { cov = 0; covWhy = name + '’s availability sources refused' + refusedWhy(row); }
    else { cov = 0; covWhy = 'no availability read covers ' + name; }
    ...
    if (pers && pStatus === 'ASSESSED' && num(pers.impact) != null) {
      var qU = 0;
      ...
      var cost = Math.min(1.5, 1.5 * pers.impact / 50 + 0.75 * Math.min(1, qU));
      imp = 1.5 - cost;
      ...
      if (cost > 0.05) impWhy = name + ' non-QB absences rate ' + pers.impact + '/100 team impact'
        ...
        + ' — measured, not priced into the spread';
    ...
    else if (pStatus === 'UNRATED_ABSENCES') { act = 'Rate the absent ' + name + ' players (player-quality attribution)'; key = 'attribution:' + side; }
  ...
  R.score = function (input, over) {
    ...
    var stab = input.stability || R.stability(c.p, { params: input.params, blend: input.blend,
      fbs: { home: !c.floorPriced('home'), away: !c.floorPriced('away') } }, over);
    var W = cfg.weights;
    var comps = {
