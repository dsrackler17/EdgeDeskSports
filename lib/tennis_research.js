...
(function (root, factory) {
  ...
  /* TWO SOURCES OF IDENTITY, ONE INDEX.

     tennis.players is the licensed record and is the authority. The provider
     directory fills the gap while that record is empty. Merged naively they
     would fight: the same player present in both would appear twice under one
     name, and the resolver would rightly call that ambiguous and refuse — so
     adding the directory would have made resolution WORSE for exactly the
     players the record already knew.

     So a directory entry is dropped wherever the licensed record already has
     that name. The record wins per player, not wholesale, and the day a
     cleared feed is loaded it simply takes over name by name. */
  /* IS THIS AN ATHLETE ID AT ALL.

     The provider's athlete ids are positive integers in a single global
     namespace. It also fields NON-POSITIVE ids for entrants who are not yet
     a person — a qualifier, a bye, a to-be-decided slot. Those are not
     identities: the same id turns up on both tours in the same week, and one
     provider id cannot be two people.

     Treating one as an identity is the worst failure this pipeline has: every
     placeholder in every draw would collapse into a single player, and every
     side carrying it would resolve to that one row. So a non-positive id is
     refused everywhere an identity is minted or matched, and the side simply
     stays unresolved — which is the truth about a slot nobody has won yet. */
  function isProviderAthleteId(id) {
  ...
  function mergePlayerSources(licensed, directory) {
    var out = [], taken = {};
    (licensed || []).forEach(function (p) {
  ...
  function resolvePlayer(name, providerId, index, aliases) {
    ...
    var k = normName(name);
    if (isProviderAthleteId(providerId)) {
  ...
  return {
    ...
    buildPlayerIndex: buildPlayerIndex, mergePlayerSources: mergePlayerSources, resolvePlayer: resolvePlayer,
    isProviderAthleteId: isProviderAthleteId,
