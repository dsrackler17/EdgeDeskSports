/* ===========================================================================
   ...
   CANDIDATE (built by the adapter; anything missing stays null)
     ...
     confidence_tier, reliability_tier, stability_tier
                     OPTIONAL: the CFB research view's tiers for the game
                     (lib/cfb_research_view.js — confidence HIGH|MODERATE|LOW,
                     reliability STRONG|ADEQUATE|LOW, stability HIGH|MEDIUM|
                     LOW). They scale the score (P.TRUST); a game that carries
                     none — every NFL game — is scored exactly as before
   =========================================================================== */
(function (root, factory) {
  ...
  /* TRUST — how far the data under a disagreement can be believed. The base
     score of a game that carries the research view's tiers is multiplied by
     these, so a large gap on thin data cannot outrank a smaller one on good
     data by size alone. The TIERS are computed in lib/cfb_research_view.js;
     only the factors live here, and a tier a game does not carry scales
     nothing. */
  P.TRUST = {
    confidence: { HIGH: 1, MODERATE: 0.85, LOW: 0.6 },
    reliability: { STRONG: 1, ADEQUATE: 0.9, LOW: 0.7 },
    stability: { HIGH: 1, MEDIUM: 0.95, LOW: 0.85 }
  };
  ...
  P.RULE = 'Reading order, not a ranking of bets. Score (0-100, not a probability): '
    ...
    + P.PENALTY.missing_input + ' per missing total. Where a game carries them (college), the part before '
    + 'the penalties is scaled by confidence (High 1, Moderate 0.85, Low 0.6), reliability (Strong 1, '
    + 'Adequate 0.9, Low 0.7) and projection stability (High 1, Medium 0.95, Low 0.85). Ties: more '
    + 'independent flags, larger normalized gap, better completeness, fresher quote, earlier kickoff.';
  ...
  P.trust = function (c) {
    var f = 1, parts = {};
    [['confidence', 'confidence_tier'], ['reliability', 'reliability_tier'], ['stability', 'stability_tier']].forEach(function (x) {
      var t = c ? c[x[1]] : null, v = (t != null && P.TRUST[x[0]][t] != null) ? P.TRUST[x[0]][t] : null;
      parts[x[0]] = v;
      if (v != null) f *= v;
    });
    return { factor: Math.round(f * 1000) / 1000, parts: parts };
  };
  ...
  P.score = function (c) {
    ...
    var tr = P.trust(c);
    return {
      score: round1(base * tr.factor - pu - pm), base: round1(base), parts: parts, omitted: omitted,
      trust: tr, penalties: { uncertainty: pu, missing_data: pm }, missing: missing,
