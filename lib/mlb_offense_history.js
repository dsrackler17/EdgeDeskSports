  ...
  function createService(opts) {
    ...
    async function changes(o) {
      ...
      try {
        ...
        var a = (await q(QUERIES.leaderboard({ season: from, min_pa: minPa, metric: metric, limit: LIMITS.leaderboard })))
          .map(shapeSeason);
        var b = (await q(QUERIES.leaderboard({ season: to, min_pa: minPa, metric: metric, limit: LIMITS.leaderboard })))
          .map(shapeSeason);
        var byId = {};
