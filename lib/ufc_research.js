...
(function (root, factory) {
  ...
  /* first and last token only: "Jose Miguel Delgado" and "Jose Delgado" share one */
  function firstLastKey(n) { var t = tokens(n); return t.length >= 2 ? t[0] + ' ' + t[t.length - 1] : ''; }
  /* The provider's stand-ins for a fighter not yet announced. Not a name, so
     never resolved and never reported as an unmatched fighter. */
  var PLACEHOLDER_RE = /^(tba|tbd|opponent tba|opponent tbd|to be announced|to be determined)$/;
  function isPlaceholderName(n) { return PLACEHOLDER_RE.test(normName(n)); }
  /* Two spellings of one person: identical once folded, or the same first and
     last token when one of them carries a middle name. "Jose Delgado" is
     "Jose Miguel Delgado"; "Chris Smith" is not "Colby Smith". */
  ...
  function buildFighterIndex(fighters) {
    var ix = { byId: {}, byName: {}, bySurname: {}, byFirstLast: {}, n: 0 };
    (fighters || []).forEach(function (f) {
      ...
      var fl = firstLastKey(f.full_name);
      if (fl) (ix.byFirstLast[fl] = ix.byFirstLast[fl] || []).push(f);
      ix.n++;
    });
    return ix;
  }
  ...
  function resolveFighter(name, providerId, index, aliases) {
    ...
    var k = normName(name);
    if (isPlaceholderName(name)) { out.method = 'placeholder'; return out; }
    ...
    /* First and last token, when the name or the row carries a middle name
       and exactly one fighter shares that pair. "Jose Miguel Delgado" ->
       Jose Delgado. Two people sharing the pair is a refusal, not a guess. */
    var fl = firstLastKey(k);
    if (fl && fl !== k) {
      var pair = (index.byFirstLast && index.byFirstLast[fl]) || [];
      if (pair.length === 1) { out.fighter_id = String(pair[0].fighter_id); out.method = 'first_last'; return out; }
      if (pair.length > 1) { out.method = 'ambiguous'; out.candidates = pair.map(function (f) { return String(f.fighter_id); }); return out; }
    } else if (fl && index.byFirstLast) {
      var rows = (index.byFirstLast[fl] || []).filter(function (f) { return tokens(f.full_name).length >= 3; });
      if (rows.length === 1) { out.fighter_id = String(rows[0].fighter_id); out.method = 'first_last'; return out; }
      if (rows.length > 1) { out.method = 'ambiguous'; out.candidates = rows.map(function (f) { return String(f.fighter_id); }); return out; }
    }
  ...
  function linkFixture(n, bouts, resolve) {
    ...
    /* 1. the bout's own names — identical once folded, or one spelling with a
       middle name the other omits */
    for (var i = 0; i < bouts.length; i++) {
      var b = bouts[i];
      if (!normName(b.red_name) || !normName(b.blue_name)) continue;
      if (sameName(n.home_team, b.red_name) && sameName(n.away_team, b.blue_name)) return made(b, 'both_names_exact', 'red', 'blue');
      if (sameName(n.home_team, b.blue_name) && sameName(n.away_team, b.red_name)) return made(b, 'both_names_exact', 'blue', 'red');
    }
  ...
  return {
    ...
    normName: normName, tokens: tokens, surnameKey: surnameKey, swappedKey: swappedKey, firstLastKey: firstLastKey, sameName: sameName, isPlaceholderName: isPlaceholderName,
