...
(function (root, factory) {
      ...
      sections.push({ key: 'trends', title: 'Player trends',
        items: (i.movers || []).slice(0, 8).map(function (p) {
          return { headline: p.full_name + ' ' + (p.delta > 0 ? 'up' : 'down') + ' ' + Math.abs(round(p.delta, 1)) + ' rating points',
                   detail: (p.window_days || 30) + '-day move, ' + (p.sample || 0) + ' matches on file, '
                         + uncertaintyPhrase(p.uncertainty) + '.',
    ...
    if ((i.uncertain || []).length) {
      sections.push({ key: 'uncertainty', title: 'Uncertainty watch',
        items: (i.uncertain || []).slice(0, 6).map(function (p) {
          return { headline: p.full_name + ' \u2014 ' + uncertaintyPhrase(p.uncertainty),
  ...
  /* HOW AN UNCERTAINTY IS SAID OUT LOUD.

     The stored uncertainty is 1 - min(sample / full_sample, 1), so it reaches
     exactly 0 the moment a player has forty rated matches — and a brief that
     printed "uncertainty 0%" beside a name was making the flat claim that
     EdgeDesk knows this player perfectly. It does not. What the zero actually
     means is narrower and worth saying: the SAMPLE has stopped being the
     limiting factor. Everything else that can be wrong about a rating — the
     Elo model, the K factor, who they happened to draw — is still wrong, and
     is not measured by this number.

     So zero is reported as what it is rather than as certainty. */
  function uncertaintyPhrase(u) {
    var v = num(u);
    if (v == null) return 'uncertainty unknown';
    if (v <= 0.001) return 'sample is no longer the limiting factor';
    return 'uncertainty ' + Math.round(v * 100) + '%';
  }
  ...
  return {
    ...
    uncertaintyPhrase: uncertaintyPhrase
  };
});
