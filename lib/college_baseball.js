...
(function (root, factory) {
  ...
  var LIMITS = { board: 400, h2h: 40, form: 25, teams: 500,
    /* Division I is a few hundred clubs. One read of the whole season table
       is smaller than one read per club on a thirty-game board, which is why
       seasonTable() exists at all. */
    seasonTable: 500,
  ...
  function createService(opts) {
    ...
    function classify(e) {
      ...
      /* A 406 PGRST106 is the cbb schema missing from PostgREST's db-schemas,
         which is a project setting and not a missing contract. The old rule
         matched the word "schema" anywhere and called every one of them
         NOT_INSTALLED, so the college card asked for SQL that was already in
         place. Check the exposure case first, and match it on the code rather
         than on a word that appears in both messages. */
      if (/PGRST106|Invalid schema|unknown schema|3F000/i.test(m)) return 'NOT_EXPOSED';
    ...
    return {
      ...
      /* THE SEASON TABLE — every club's folded season row for one season,
         with the conference it plays in carried beside it, the games-weighted
         league run environment, and the conference strength that environment
         is read against.

         WHY ONE READ. A three-day college card touches well over a hundred
         clubs. One request per club is a board nobody waits for, and it would
         also make the league mean depend on which games happened to be on the
         card. This reads the table once and every club is measured against
         the same league.

         WHY THE LEAGUE MEAN IS GAMES-WEIGHTED. It is the sum of runs over the
         sum of games, not the mean of club rates. A club nine games into its
         season should not count as much as one fifty games in, and an
         unweighted mean lets a handful of tiny samples move the baseline
         everything else is judged against.

         CONFERENCE STRENGTH, AND WHY IT IS COMPUTABLE AT ALL. Games inside a
         conference cancel: a run one member scores is a run another member
         allows, so the sum of run differentials across a conference is zero
         over its internal games. Whatever differential remains was earned
         OUTSIDE the conference, and dividing it by the non-conference games
         played turns it into runs per non-conference game. It is folded from
         the same table the records come from — no poll, no rating service,
         and no second source to disagree with. It is reported with the sample
         it rests on so a conference with almost no non-conference play can be
         shown as unmeasured rather than trusted. */
      seasonTable: async function (season, force) {
        season = int(season);
        if (!season) return fail('NO_SEASON', 'A season table needs a season.');
        var ck = 'seasonTable:' + season;
        if (!force && cache[ck]) return cache[ck];
        try {
          var rows = await read('team_seasons',
            'select=' + COLS.teamSeasons + '&season=eq.' + season
            + '&order=team_id.asc&limit=' + LIMITS.seasonTable);
          var clubs = (rows || []).map(shapeTeamSeason).filter(Boolean);
          if (!clubs.length) {
            return fail('EMPTY_SEASON',
              'No club has a folded season row for ' + season + ', so there is no league to measure against.',
              { season: season });
          }
          /* The conference list is best effort on purpose: a club whose
             conference is unknown keeps its own rates and simply earns no
             schedule adjustment, which is the honest outcome rather than a
             missing board. */
          var teams = [];
          try { teams = await read('teams', 'select=' + COLS.teams + '&limit=' + LIMITS.teams); }
          catch (e2) { teams = []; }
          var confOf = {};
          (teams || []).forEach(function (t) {
            if (t && t.team_id != null) confOf[String(t.team_id)] = t.conference_name || null;
          });

          var runs = 0, allowed = 0, games = 0, byId = {}, conf = {};
          clubs.forEach(function (c) {
            c.conference_name = confOf[c.team_id] || null;
            byId[c.team_id] = c;
            if (c.games > 0) { runs += c.runs_for; allowed += c.runs_against; games += c.games; }
            if (!c.conference_name || !c.games) return;
            var a = conf[c.conference_name] || (conf[c.conference_name] = {
              name: c.conference_name, clubs: 0, games: 0, nonconference_games: 0, run_diff: 0
            });
            var confG = c.conference.wins + c.conference.losses;
            a.clubs++; a.games += c.games;
            a.nonconference_games += Math.max(0, c.games - confG);
            a.run_diff += (c.runs_for - c.runs_against);
          });
          Object.keys(conf).forEach(function (k) {
            var a = conf[k];
            a.runs_per_nonconf_game = a.nonconference_games > 0 ? a.run_diff / a.nonconference_games : null;
            a.sufficient = a.nonconference_games >= 20 && a.runs_per_nonconf_game != null;
          });

          var out = { ok: true, season: season, clubs: clubs, by_id: byId,
            conferences: conf,
            league: {
              clubs: clubs.filter(function (c) { return c.games > 0; }).length,
              games: games,
              /* games here counts each club-game once, so runs/games is runs
                 per team per game — the same grain a projection needs. */
              runs_per_game: games > 0 ? runs / games : null,
              runs_allowed_per_game: games > 0 ? allowed / games : null
            },
            note: 'Folded from cbb.team_seasons, which is itself folded from the game log. The league rate is '
              + 'games-weighted, so a club nine games into its season does not count as much as one fifty games in.' };
          cache[ck] = out;
          return out;
        } catch (e) { return fail(classify(e), String((e && e.message) || e)); }
      },
      ...
      /* THE BOARD. Every game in a window, earliest first. Out of season it
         ...
         AND OUT OF SEASON IT NOW FALLS BACK, because "correct" and "useful"
         were not the same thing here. College baseball runs February to June,
         so for seven months of the year the card was empty and the panel said
         so — accurate, and it meant the whole surface did nothing from July to
         January while a complete season sat in the archive underneath it.

         With q.fallback, an empty window is answered with the most recent day
         that HAS games, so the card, the briefs and the research behind them
         keep working out of season. The response never pretends that day is
         today: requested_from and requested_through are what was asked for,
         fell_back_to is the day actually shown, and a caller that does not
         pass q.fallback gets exactly the old behaviour. */
      board: async function (q) {
        ...
        try {
          ...
          if (!games.length && q.fallback) {
            /* The last day of baseball actually played on or before the window.
               Two reads rather than one scan: find the date, then take that
               day. A season with no games at all leaves this null and the
               caller gets the ordinary empty board. */
            var last = await read('games',
              'select=game_date&game_date=lte.' + through
              + '&order=game_date.desc&limit=1');
            var lastDay = last && last[0] && last[0].game_date
              ? String(last[0].game_date).slice(0, 10) : null;
            if (lastDay) {
              var back = await read('games',
                'select=' + COLS.games
                + '&game_date=eq.' + lastDay
                + '&order=start_time.asc&limit=' + LIMITS.board);
              var backGames = (back || []).map(shapeGame).filter(Boolean);
              if (backGames.length) {
                games = backGames;
                fellBackTo = lastDay;
              }
          ...
          return { ok: true,
            from: fellBackTo || from, through: fellBackTo || through,
            requested_from: from, requested_through: through,
            fell_back_to: fellBackTo,
            season_shown: fellBackTo ? seasonOf(fellBackTo) : null,
      ...
      /* ── THE ARCHIVE, REACHED BY THE ID THE GAMES BOARD USES ─────────────
         A game brief knows ESPN team ids. The archive knows NCAA club codes.
         cbb.club_map joins them, and cbb.archive_by_espn_club is an INNER join
         over it — so a club with no mapping yields no rows rather than somebody
         else's, which is the only acceptable failure mode here.

         310 of 311 clubs are mapped. The one that is not returns NOT_MAPPED and
         a sentence a reader can act on, because "no archive shown" and "this
         club's archive is missing" are different claims and only one of them is
         true. */
      archiveForClub: async function (espnTeamId, season) {
        if (!espnTeamId || !season) {
          return fail('NO_TEAM', 'An ESPN team id and a season are needed.');
        }
        try {
          var rows = await read('archive_by_espn_club',
            'select=*&espn_team_id=eq.' + encodeURIComponent(espnTeamId)
          ...
          if (!players.length) {
            return fail('NOT_MAPPED', 'This club is not linked to the NCAA season archive, so '
              + 'no complete-season batting or pitching line is shown for it. Its record and '
              + 'run rates above are unaffected — those come from the game log. Of 311 clubs '
              + 'in the archive, 310 are linked; this is either the one that is not, or a club '
              + 'with no players in the archive for ' + season + '.');
          }
          /* Club totals, summed here rather than in SQL because this is one
             club's roster already in hand and a second round trip to add up
             sixty rows would be a round trip for nothing. */
          var sum = function (pick) {
            return players.reduce(function (n, p) { var v = pick(p); return n + (v || 0); }, 0);
          };
          var ab = sum(function (p) { return p.batting && p.batting.ab; });
          var h = sum(function (p) { return p.batting && p.batting.h; });
          var bb = sum(function (p) { return p.batting && p.batting.bb; });
          var hbp = sum(function (p) { return p.batting && p.batting.hbp; });
          var sf = sum(function (p) { return p.batting && p.batting.sf; });
          var tb = sum(function (p) { return p.batting && p.batting.total_bases; });
          var outs = sum(function (p) { return p.pitching && p.pitching.outs; });
          var er = sum(function (p) { return p.pitching && p.pitching.earned_runs; });
          var ph = sum(function (p) { return p.pitching && p.pitching.hits_allowed; });
          var pbb = sum(function (p) { return p.pitching && p.pitching.bb; });
          var obDen = ab + bb + hbp + sf;
          return { ok: true, espn_team_id: String(espnTeamId), season: int(season),
            ncaa_code: (rows[0] || {}).ncaa_code || null,
            mapped_via: (rows[0] || {}).mapped_via || null,
            ...
            /* Every one of these divides by a real denominator or is null. The
               club OBP is computable here and NOT computable from box scores,
               which is the difference the whole archive exists for. */
            totals: {
              ab: ab, h: h, bb: bb, hbp: hbp, sf: sf, total_bases: tb,
              hr: sum(function (p) { return p.batting && p.batting.hr; }),
              rbi: sum(function (p) { return p.batting && p.batting.rbi; }),
              so: sum(function (p) { return p.batting && p.batting.so; }),
              sb: sum(function (p) { return p.batting && p.batting.sb; }),
              avg: ab > 0 ? h / ab : null,
              avg_text: ab > 0 ? rateText(h / ab) : null,
              obp: obDen > 0 ? (h + bb + hbp) / obDen : null,
              obp_text: obDen > 0 ? rateText((h + bb + hbp) / obDen) : null,
              slg: ab > 0 ? tb / ab : null,
              slg_text: ab > 0 ? rateText(tb / ab) : null,
              outs: outs, ip: ipText(outs),
              era: outs > 0 ? er * 27 / outs : null,
              whip: outs > 0 ? (ph + pbb) * 3 / outs : null,
              so_pitching: sum(function (p) { return p.pitching && p.pitching.so; }),
            } };
        } catch (e) { return fail(classify(e), String((e && e.message) || e)); }
      },
