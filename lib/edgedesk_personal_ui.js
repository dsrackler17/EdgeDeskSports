...
(function () {
  ...
  function maybeOnboard() {
    ...
    (function wait() {
      var wel = $('welHost');
      if (wel && wel.classList.contains('on') && tries++ < 240) { setTimeout(wait, 1000); return; }
      M.onboard(1);
    })();
  }
  ...
  M.onboard = function (step) {
    ...
    if (step === 1) body = '<h3>What do you research?</h3><p class="edm-sub">Your board, Top 5 and alerts start here. You can change this any time in Settings.</p>'
      ...
      body = '<h3>Research alerts <span class="edm-sub">optional</span></h3><p class="edm-sub">EdgeDesk alerts on research conditions for games you watch — never on "bets". Defaults are sensible; change them later.</p>'
        ...
        + '</div><label class="edm-cb"><input type="checkbox" id="onbQb"' + (a.on_qb_confirmed ? ' checked' : '') + '> A watched game’s QB is confirmed</label>'
        + '<label class="edm-cb"><input type="checkbox" id="onbInj"' + (a.on_injury_change ? ' checked' : '') + '> A major availability change</label>'
        + '<label class="edm-cb"><input type="checkbox" id="onbScope"' + (a.scope === 'leagues' ? ' checked' : '') + '> Also tell me when any game in my leagues becomes research-grade</label>';
