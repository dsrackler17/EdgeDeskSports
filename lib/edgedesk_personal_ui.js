/* ===========================================================================
   ...
   quality, Compare My Number, share research cards) in the order the
   reader's persona puts them, the journal's decision form, Compare My Number,
   the research card, first-run onboarding (its first question: "What best
   describes how you research?"), the settings sections (research
   preferences, research alerts, partner program with its campaigns), the
   affiliate attribution claim, the acquisition claim, and the trial-action
   events (supabase/growth.sql) — sent, never read back or shown.
...
(function () {
  ...
  var S = M.S = {
    ...
    liveAt: 0, live: {},
    affiliate: null, cards: null, cardRecorded: {}
  };
  ...
  M.boot = async function () {
    ...
    if (S.schema) { importLocalFollows(); maybeOnboard(); M.track('visit'); }
    claimAttribution();
    claimAcquisition();
  ...
  function watchRowHTML(w, full) {
    ...
    var acts = '<div class="edm-acts"><button class="edm-btn" onclick="EDMine.openGame(' + js(key) + ')">Research matchup</button>'
      + (started ? '' : '<button class="edm-btn ghost" onclick="EDMine.journalOpen(' + js(key) + ')">Log decision</button>'
        + '<button class="edm-btn ghost" onclick="EDMine.compareOpen(' + js(key) + ')">Compare my number</button>'
        + '<button class="edm-btn ghost" onclick="EDMine.shareOpen(' + js(key) + ')">Share card</button>') + '</div>';
  ...
  /* ------------------------------------------------------------ activation
     The trial actions that write no row of their own go to public.edp_track
     (supabase/growth.sql), which decides what counts and dedupes by day.
     Watchlist saves, journal entries, alert settings and share cards are
     recorded by the database from their own rows. Nothing about the reader's
     activation is ever read back or shown to them. */
  M._tracked = {};
  M.track = function (kind, gameKey) {
    if (!P || !signedIn() || S.schema === false) return;
    var k = kind + '|' + (gameKey || '') + '|' + new Date().toISOString().slice(0, 10);
    if (M._tracked[k]) return;
    M._tracked[k] = 1;
    try { rpc('edp_track', { p_kind: kind, p_game_key: gameKey || null }).catch(function () { }); } catch (_) { }
  };
  M.trackGame = function (sport, gid) { if (P && gid != null && gid !== '') M.track('matchup_viewed', P.gameKey(sport, gid)); };

  M.openGame = function (key) {
    var p = String(key).split('|');
    M.track('matchup_viewed', key);
  ...
  function topRowHTML(s, i) {
    ...
    return '<div class="edm-top">'
      ...
      + '<button class="edm-btn ghost" onclick="EDMine.journalOpen(' + js(s.game_key) + ')">Log decision</button>'
      + '<button class="edm-btn ghost" onclick="EDMine.compareOpen(' + js(s.game_key) + ')">Compare my number</button>'
      + '<button class="edm-btn ghost" onclick="EDMine.shareOpen(' + js(s.game_key) + ')">Share card</button></div></div>';
  }
  ...
  /* THE ORDER is the reader's persona's (EDPersonal.personaPlan): one
     section moves to the front, none is ever removed. */
  function sectionHTML(k) {
    if (k === 'top5') return topHTML();
    if (k === 'watchlist') return watchDeskHTML();
    if (k === 'changes') return changesDeskHTML();
    if (k === 'quality') return perfDeskHTML();
    if (k === 'compare') return compareDeskHTML();
    if (k === 'share') return shareDeskHTML();
    return '';
  }
  function renderDesk() {
    var top = $('edmDeskTop'), bottom = $('edmDeskBottom');
    var plan = P.personaPlan(S.prefs && S.prefs.persona);
    if (top) top.innerHTML = personaPromptHTML()
      + (plan.note ? '<div class="edm-spot">Arranged for how you research — <b>' + esc(P.personaOf(plan.persona).label) + '</b>. ' + esc(plan.note)
        + ' <button class="edm-info" onclick="EDMine.personaOpen()">Change</button></div>' : '')
      + plan.top.map(sectionHTML).join('');
    if (bottom) bottom.innerHTML = plan.bottom.map(sectionHTML).join('');
    paintStars();
    if (document.querySelector('#edmTop5 .edm-top')) M.track('top5_opened');
  }
  M.paintDesk = async function () {
    ...
    if (!S.cards) need.push(loadCards());
  ...
  /* ------------------------------------------------------------ persona
     "What best describes how you research?" — asked once in onboarding, and
     once on the desk for a reader who finished onboarding before it existed.
     The answer only reorders the desk (EDPersonal.personaPlan). */
  function personaChips(sel, fn) {
    return '<div class="edm-chips" role="radiogroup" aria-label="How you research">' + P.PERSONAS.map(function (x) {
      var on = sel === x.key;
      return '<button type="button" role="radio" aria-checked="' + on + '" class="edm-chip' + (on ? ' on' : '') + '" onclick="' + fn + '(' + js(x.key) + ')">' + esc(x.label) + '</button>';
    }).join('') + '</div>';
  }
  function personaPromptHTML() {
    if (!S.prefs || S.prefs.persona || S.prefs.onboarding_status === 'pending') return '';
    try { if (localStorage.getItem('edgedesk_persona_prompt_v1')) return ''; } catch (_) { }
    return '<section class="edm-sec edm-persona" id="edmPersonaAsk"><div class="edm-sec-h"><h3>What best describes how you research?</h3>'
      + '<span class="edm-sub">one question · it only changes what your desk shows first</span></div>'
      + personaChips(null, 'EDMine.personaSet')
      + '<div class="edm-acts"><button class="edm-btn ghost sm" onclick="EDMine.personaDismiss()">Not now</button></div></section>';
  }
  M.personaDismiss = function () { try { localStorage.setItem('edgedesk_persona_prompt_v1', String(Date.now())); } catch (_) { } renderDesk(); };
  M.personaSet = async function (key) {
    if (!P.personaOf(key)) return;
    var prev = S.prefs;
    S.prefs = Object.assign({}, S.prefs || { leagues: [], books: [], interests: [] }, { persona: key });
    M.modalClose('edmPersona'); renderDesk(); rerender();
    try {
      var r = await api('user_preferences?on_conflict=user_id', { method: 'POST', body: [{ persona: key }], prefer: 'return=representation,resolution=merge-duplicates' });
      if (r && r[0]) S.prefs = r[0];
      toast('Your desk is arranged for: ' + P.personaOf(key).label + '. Everything else is still here.');
      event('persona_set', { persona: key });
    } catch (_) { S.prefs = prev; renderDesk(); rerender(); toast('That did not save. Try again.', true); }
  };
  M.personaOpen = function () {
    var host = modalHost('edmPersona'), cur = S.prefs && S.prefs.persona;
    host.innerHTML = '<div class="edm-modal" role="dialog" aria-modal="true" aria-labelledby="edmPT"><div class="edm-mh"><h3 id="edmPT">What best describes how you research?</h3>'
      + '<button class="edm-x" onclick="EDMine.modalClose(\'edmPersona\')" aria-label="Close">×</button></div>'
      + '<p class="edm-sub">It decides what your desk shows first. Nothing is hidden: every section stays one scroll away.</p>'
      + personaChips(cur, 'EDMine.personaSet') + '</div>';
    host.classList.add('on');
  };

  /* games a reader can act on from the desk: the Top 5 of the league on
     screen, then upcoming watched games */
  function actionableGames(n) {
    var out = [], seen = {};
    function add(st) { if (st && st.game_key && !seen[st.game_key]) { var k = Date.parse(st.kickoff_at); if (!isFinite(k) || k > Date.now()) { seen[st.game_key] = 1; out.push(st); } } }
    var lgs = leagues();
    lgs.forEach(function (lg) { topFor(lg).list.forEach(add); });
    (S.watchLoaded ? S.watchRows : []).forEach(function (w) { add(M.stateFor(w.game_key) || w.state); });
    return out.slice(0, n || 6);
  }
  function miniGameHTML(st, btn) {
    var f = st.fair || {}, m = st.market || {};
    return '<div class="edm-mini"><div class="edm-mini-t"><b>' + esc(P.matchup(st)) + '</b><span class="edm-sub">' + esc(st.sport === 'nfl' ? 'NFL' : 'CFB') + ' · ' + esc(when(st.kickoff_at)) + '</span></div>'
      + '<div class="edm-mini-n"><span class="mdl">EdgeDesk ' + esc(f.text || P.favText(st, f.home_line) || '—') + '</span><span>Market ' + esc(m.stale ? 'stale' : (m.text || P.favText(st, m.home_line) || 'none')) + '</span></div>'
      + btn + '</div>';
  }

  /* ------------------------------------------------------------ Compare My Number */
  function hasNumber(e) { return num(e.my_home_line) != null || num(e.my_total) != null; }
  function myNumberText(e) {
    var s0 = { home: e.home || 'Home', away: e.away || 'Away' }, bits = [];
    if (num(e.my_home_line) != null) bits.push(P.favText(s0, e.my_home_line));
    if (num(e.my_total) != null) bits.push('total ' + e.my_total);
    return bits.join(' · ');
  }
  function compareDeskHTML() {
    var j = S.journal || [], saved = j.filter(hasNumber), ns = P.numbersSummary(j), games = actionableGames(5);
    var body = games.length ? games.map(function (st) { return miniGameHTML(st, '<button class="edm-btn sm" onclick="EDMine.compareOpen(' + js(st.game_key) + ')">Compare my number</button>'); }).join('')
      : '<div class="edm-empty">Open any game, or load a slate, and <b>Compare my number</b> sets your fair spread and total beside EdgeDesk’s and the market’s.</div>';
    var recent = saved.slice(0, 4).map(function (e) {
      var vc = P.numbersVsClose(e), s0 = { home: e.home || 'Home', away: e.away || 'Away' };
      return '<li><b>' + esc((e.away || '?') + ' @ ' + (e.home || '?')) + '</b> — yours ' + esc(myNumberText(e))
        + (num(e.snap_fair_home_line) != null ? ' · EdgeDesk then ' + esc(P.favText(s0, e.snap_fair_home_line)) : '')
        + (vc.close_home_line != null && vc.mine != null ? ' · close ' + esc(P.favText(s0, vc.close_home_line)) : '')
        + ' <span class="edm-sub">' + esc(ago(e.created_at)) + '</span></li>';
    }).join('');
    return '<section class="edm-sec" id="edmCompare"><div class="edm-sec-h"><h3>Compare My Number</h3><span class="edm-sub">your number beside EdgeDesk’s and the market’s</span>'
      + '<span class="edm-sp"></span>' + (saved.length ? '<button class="edm-info" onclick="EDMine.open(\'journal\')">Your saved numbers (' + saved.length + ')</button>' : '') + '</div>'
      + body
      + (recent ? '<div class="edm-ex"><b>Your recent numbers</b><ul>' + recent + '</ul></div>' : '')
      + (ns.graded ? '<div class="edm-note">' + ns.graded + ' of your numbers have a close on file; see Decision quality for how far each number sat from it.</div>' : '')
      + '<div class="edm-foot">Neither number is declared right. EdgeDesk shows where you agree, where you differ, and which of its measured inputs the difference runs through.</div></section>';
  }
  M.compareOpen = async function (key) {
    if (!signedIn()) { toast('Sign in to compare your number.', true); return; }
    var s = M.stateFor(key) || await stateFromServer(key);
    var host = modalHost('edmCModal');
    M._cState = { key: key, state: s };
    if (!S.journal && S.schema) { M.loadJournal().then(function () { if (M._cState && M._cState.key === key) paintCmpHistory(); }); }
    if (!s) {
      host.innerHTML = '<div class="edm-modal" role="dialog" aria-modal="true" aria-label="Compare my number"><div class="edm-mh"><h3>Compare my number</h3><button class="edm-x" onclick="EDMine.modalClose(\'edmCModal\')" aria-label="Close">×</button></div>'
        + '<div class="edm-empty">EdgeDesk holds no research state for this game right now, so there is nothing to compare your number with. It appears once the game is on a board EdgeDesk has read.</div></div>';
      host.classList.add('on'); return;
    }
    var fe = num(s.fair && s.fair.home_line), favSide = fe == null ? 'home' : (fe <= 0 ? 'home' : 'away');
    host.innerHTML = '<div class="edm-modal edm-wide" role="dialog" aria-modal="true" aria-labelledby="edmCT"><div class="edm-mh"><h3 id="edmCT">Compare my number</h3><button class="edm-x" onclick="EDMine.modalClose(\'edmCModal\')" aria-label="Close">×</button></div>'
      + '<div class="edm-sub">' + esc(P.matchup(s) + ' · ' + when(s.kickoff_at)) + '</div>'
      + '<div class="edm-f2 edm-cmp-in">'
      + '<div class="edm-f"><label for="edmCLine">Your fair spread</label><div class="edm-inrow"><select id="edmCTeam" aria-label="Team your spread is for" onchange="EDMine.cmpRun()">'
      + ['away', 'home'].map(function (k) { return '<option value="' + k + '"' + (k === favSide ? ' selected' : '') + '>' + esc(s[k]) + '</option>'; }).join('')
      + '</select><input id="edmCLine" inputmode="decimal" autocomplete="off" placeholder="-3.5 or PK" oninput="EDMine.cmpRun()"></div></div>'
      + '<div class="edm-f"><label for="edmCTotal">Your fair total <span class="edm-sub">optional</span></label><input id="edmCTotal" inputmode="decimal" autocomplete="off" placeholder="e.g. 47.5" oninput="EDMine.cmpRun()"></div>'
      + '</div>'
      + '<div id="edmCOut" aria-live="polite"></div>'
      + '<div id="edmCHist"></div>'
      + '<div class="edm-f2"><div class="edm-f"><label for="edmCDec">Save as</label><select id="edmCDec"><option value="researching">Researching</option><option value="passed">Passed</option><option value="leaned">Leaned</option></select></div>'
      + '<div class="edm-f"><label for="edmCNotes">Notes <span class="edm-sub">optional, only you can read them</span></label><input id="edmCNotes" maxlength="4000" placeholder="What your number sees that EdgeDesk’s does not"></div></div>'
      + '<div class="edm-err" id="edmCErr" role="alert"></div>'
      + '<div class="edm-acts"><button class="edm-btn" id="edmCSave" disabled onclick="EDMine.cmpSave()">Save my number to my journal</button>'
      + '<button class="edm-btn ghost" onclick="EDMine.modalClose(\'edmCModal\')">Close</button></div>'
      + '<div class="edm-foot">Saved with EdgeDesk’s numbers as they stand now, and never changed afterwards — a new number is a new entry, so every number you enter is kept. After kickoff the close is recorded beside it.</div></div>';
    host.classList.add('on');
    paintCmpHistory(); M.cmpRun();
    try { $('edmCLine').focus(); } catch (_) { }
    event('compare_open', { game_key: key });
  };
  function paintCmpHistory() {
    var el = $('edmCHist'), st = M._cState; if (!el || !st) return;
    var prev = (S.journal || []).filter(function (e) { return e.game_key === st.key && hasNumber(e); });
    el.innerHTML = prev.length ? '<div class="edm-ex"><b>Your earlier numbers for this game</b><ul>' + prev.slice(0, 6).map(function (e) {
      var s0 = { home: e.home || 'Home', away: e.away || 'Away' };
      return '<li>' + esc(myNumberText(e)) + (num(e.snap_fair_home_line) != null ? ' — EdgeDesk then ' + esc(P.favText(s0, e.snap_fair_home_line)) : '')
        + (num(e.snap_market_home_line) != null ? ', market then ' + esc(P.favText(s0, e.snap_market_home_line)) : '') + ' <span class="edm-sub">' + esc(when(e.created_at)) + '</span></li>';
    }).join('') + '</ul></div>' : '';
  }
  function cmpRowHTML(label, you, ed, mk) {
    return '<tr><th scope="row">' + esc(label) + '</th><td class="you">' + esc(you == null ? '—' : you) + '</td><td class="mdl">' + esc(ed == null ? '—' : ed) + '</td><td>' + esc(mk == null ? '—' : mk) + '</td></tr>';
  }
  function listHTML(title, items, cls) {
    return items.length ? '<div class="edm-ex' + (cls ? ' ' + cls : '') + '"><b>' + esc(title) + '</b><ul>' + items.map(function (t) { return '<li>' + esc(t) + '</li>'; }).join('') + '</ul></div>' : '';
  }
  function cmpHTML(r, s) {
    if (!r.ok) return '<div class="edm-note">' + esc(r.reasons.join('. ')) + '.</div>';
    var sp = r.spread, tt = r.total;
    return '<table class="edm-cmp"><thead><tr><th></th><th>You</th><th>EdgeDesk</th><th>Market</th></tr></thead><tbody>'
      + (sp ? cmpRowHTML('Spread', sp.mine_text, sp.edgedesk_text, sp.market_text) : '')
      + (tt ? cmpRowHTML('Total', tt.mine, tt.edgedesk, tt.market) : '') + '</tbody></table>'
      + listHTML('The differences', (sp ? sp.lines : []).concat(tt ? tt.lines : []))
      + listHTML('Where you agree with EdgeDesk', r.agrees, 'agree')
      + listHTML('Where you differ', r.differs, 'concern')
      + listHTML('Which of EdgeDesk’s measured inputs the difference runs through', r.inputs.map(function (x) { return x.text; }))
      + listHTML('What EdgeDesk’s number knew', r.context)
      + '<div class="edm-note">' + esc(r.note) + '</div>';
  }
  function numOrNull(id) {
    var v = String(($(id) || {}).value || '').trim();
    if (!v) return null;
    return /^\d{1,3}(\.\d{1,2})?$/.test(v) ? parseFloat(v) : NaN;
  }
  M.cmpRun = function () {
    var st = M._cState, out = $('edmCOut'), save = $('edmCSave'), err = $('edmCErr');
    if (!st || !st.state || !out) return;
    var lineTxt = String(($('edmCLine') || {}).value || '').trim();
    var h = lineTxt ? P.parseTeamLine(st.state, ($('edmCTeam') || {}).value, lineTxt) : null;
    var t = numOrNull('edmCTotal');
    if (err) err.textContent = '';
    if ((h != null && !isFinite(h)) || (t != null && !isFinite(t))) {
      if (err) err.textContent = (h != null && !isFinite(h) ? 'Enter your spread as a number for the team you picked, like -3.5, +7 or PK. ' : '') + (t != null && !isFinite(t) ? 'Enter your total as a number, like 47.5.' : '');
      if (save) save.disabled = true; return;
    }
    if (h == null && t == null) {
      out.innerHTML = '<div class="edm-note">Enter your fair spread for either team, your fair total, or both. EdgeDesk shows its number and the market’s beside yours as you type.</div>';
      if (save) save.disabled = true; st.result = null; return;
    }
    var r = P.compareNumber(st.state, { home_line: h, total: t });
    out.innerHTML = cmpHTML(r, st.state);
    st.result = r; st.mine = { home_line: h, total: t };
    if (save) save.disabled = !r.ok;
    if (r.ok) M.track('compare_my_number', st.key);
  };
  M.cmpSave = async function () {
    var st = M._cState, err = $('edmCErr'), btn = $('edmCSave');
    if (!st || !st.result || !st.result.ok) return;
    var s = st.state;
    var entry = Object.assign({ game_key: st.key, decision: ($('edmCDec') || {}).value || 'researching', my_home_line: st.mine.home_line, my_total: st.mine.total,
      notes: String(($('edmCNotes') || {}).value || '').trim() || null, home: s.home, away: s.away, kickoff_at: s.kickoff_at }, P.journalSnapshot(s));
    ...
    try {
      ...
      M.modalClose('edmCModal'); toast('Your number is saved to your research journal, beside EdgeDesk’s.');
      event('compare_save', { game_key: st.key });
      ...
      if (btn) { btn.disabled = false; btn.textContent = 'Save my number to my journal'; }
    }
  };
  ...
  async function loadCards() {
    if (!S.schema) return [];
    try { S.cards = await api('share_cards?select=card_id,game_key,format,created_at,content&order=created_at.desc&limit=20') || []; }
    catch (e) { S.cards = S.cards || []; }
    return S.cards;
  }
  function shareDeskHTML() {
    var games = actionableGames(5), cards = S.cards || [];
    return '<section class="edm-sec" id="edmShare"><div class="edm-sec-h"><h3>Share research cards</h3><span class="edm-sub">sized for X · real current data only</span></div>'
      + (games.length ? games.map(function (st) { return miniGameHTML(st, '<button class="edm-btn sm ghost" onclick="EDMine.shareOpen(' + js(st.game_key) + ')">Make a card</button>'); }).join('')
        : '<div class="edm-empty">Load a slate or open a game, then <b>Make a card</b>: EdgeDesk’s fair line, the market, the gap, reliability and the evidence, as a clean image.</div>')
      + (cards.length ? '<div class="edm-note">You have made ' + cards.length + ' card' + (cards.length === 1 ? '' : 's') + '; the latest was ' + esc(cards[0].content && cards[0].content.matchup || '') + ', ' + esc(ago(cards[0].created_at)) + '.</div>' : '')
      + '<div class="edm-foot">Every card carries “Research, not picks.”, the time of EdgeDesk’s numbers and edgedesksports.com. It never states a result or tells anyone to bet.</div></section>';
  }
  M.shareOpen = async function (key) {
    if (!signedIn()) { toast('Sign in to make a research card.', true); return; }
    var SC = window.EDShareCard;
    if (!SC) { toast('The research-card layer did not load.', true); return; }
    var s = M.stateFor(key) || await stateFromServer(key);
    var r = SC.content(s), host = modalHost('edmSModal');
    var head = '<div class="edm-mh"><h3 id="edmST">Share a research card</h3><button class="edm-x" onclick="EDMine.modalClose(\'edmSModal\')" aria-label="Close">×</button></div>';
    if (!r.ok) {
      host.innerHTML = '<div class="edm-modal" role="dialog" aria-modal="true" aria-labelledby="edmST">' + head + '<div class="edm-empty">' + esc(r.message) + '</div></div>';
      host.classList.add('on'); return;
    }
    M._sState = { key: key, r: r, format: 'x_landscape' };
    host.innerHTML = '<div class="edm-modal edm-wide" role="dialog" aria-modal="true" aria-labelledby="edmST">' + head
      + '<div class="edm-seg" role="tablist" id="edmSFmt">' + Object.keys(SC.FORMATS).map(function (f, i) {
        return '<button role="tab" aria-selected="' + (i === 0) + '" class="' + (i === 0 ? 'on' : '') + '" data-f="' + f + '" onclick="EDMine.shareFormat(' + js(f) + ')">' + esc(SC.FORMATS[f].label) + '</button>';
      }).join('') + '</div>'
      + '<div class="edm-canvas"><canvas id="edmSCanvas" role="img" aria-label="' + attr('Research card: ' + r.content.matchup + ', EdgeDesk ' + r.content.fair + ', market ' + r.content.market) + '"></canvas></div>'
      + '<div class="edm-acts"><button class="edm-btn" onclick="EDMine.shareDownload()">Download image</button>'
      + (navigator.share ? '<button class="edm-btn ghost" onclick="EDMine.shareNative()">Share…</button>' : '')
      + '<button class="edm-btn ghost" onclick="EDMine.shareX()">Post on X</button><button class="edm-btn ghost" onclick="EDMine.shareCopy()">Copy text</button></div>'
      + '<div class="edm-foot">Real current data only: EdgeDesk’s research state as of ' + esc(SC.utc(r.state_computed_at) || '—') + '. '
      + 'Research, not picks — the card never states a result or tells anyone to bet. On X, attach the downloaded image to the post.</div></div>';
    host.classList.add('on');
    M.shareDraw();
    event('share_card_open', { game_key: key });
  };
  M.shareFormat = function (f) {
    var st = M._sState; if (!st) return;
    st.format = f;
    var bs = document.querySelectorAll('#edmSFmt button');
    for (var i = 0; i < bs.length; i++) { var on = bs[i].getAttribute('data-f') === f; bs[i].classList.toggle('on', on); bs[i].setAttribute('aria-selected', String(on)); }
    M.shareDraw();
  };
  M.shareDraw = function () {
    var st = M._sState, SC = window.EDShareCard, cv = $('edmSCanvas');
    if (!st || !SC || !cv) return;
    function paint() {
      try { SC.draw(cv, SC.layout(st.r.content, st.format, SC.measureWith(cv))); } catch (_) { }
      recordCard(st.format);
    }
    try { if (document.fonts && document.fonts.load) { Promise.all([document.fonts.load('800 40px Inter'), document.fonts.load('500 20px Inter')]).then(paint, paint); return; } } catch (_) { }
    paint();
  };
  /* the record of what was printed, once per card and format (the database
     refuses tout copy and a game that has kicked off) */
  async function recordCard(format) {
    var st = M._sState; if (!st || S.schema === false) return;
    var k = st.r.hash + '|' + format;
    if (S.cardRecorded[k]) return;
    S.cardRecorded[k] = 1;
    var n = st.r.numbers;
    try {
      await api('share_cards', { method: 'POST', prefer: 'return=minimal', body: [{ game_key: st.key, format: format, state_hash: st.r.state_hash,
        state_computed_at: st.r.state_computed_at, fair_home_line: n.fair_home_line, market_home_line: n.market_home_line, gap_pts: n.gap_pts,
        reliability_score: n.reliability_score, content: st.r.content, content_hash: st.r.hash }] });
      S.cards = [{ game_key: st.key, format: format, created_at: new Date().toISOString(), content: st.r.content }].concat(S.cards || []);
      event('share_card_generated', { game_key: st.key, format: format });
    } catch (e) {
      if (e && e.status === 400 && /kicked off/.test(e.body || '')) toast('This game has kicked off. Research cards are made before the game.', true);
    }
  }
  function cardFile(cb) {
    var st = M._sState, cv = $('edmSCanvas'); if (!st || !cv) return;
    var name = 'edgedesk-' + String(st.r.content.matchup).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') + '-' + st.format + '.png';
    cv.toBlob(function (b) { if (b) cb(b, name); }, 'image/png');
  }
  M.shareDownload = function () {
    cardFile(function (b, name) {
      var a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = name; document.body.appendChild(a); a.click(); a.remove();
      event('share_card_download', { format: M._sState.format });
    });
  };
  M.shareNative = function () {
    var st = M._sState; if (!st) return;
    cardFile(function (b, name) {
      var f = null; try { f = new File([b], name, { type: 'image/png' }); } catch (_) { f = null; }
      var data = { text: window.EDShareCard.shareText(st.r.content) };
      if (f && navigator.canShare && navigator.canShare({ files: [f] })) data.files = [f];
      navigator.share(data).catch(function () { });
    });
  };
  M.shareCopy = function () { var st = M._sState; if (st) M.copy(window.EDShareCard.shareText(st.r.content)); };
  M.shareX = function () {
    var st = M._sState; if (!st) return;
    window.open('https://x.com/intent/post?text=' + encodeURIComponent(window.EDShareCard.shareText(st.r.content)), '_blank', 'noopener');
    event('share_card_x', { game_key: st.key });
  };
  ...
  M.journalCsv = function () {
    var cols = ['created_at', 'game_key', 'away', 'home', 'kickoff_at', 'decision', 'market_type', 'selection', 'sportsbook', 'line', 'price_american', 'stake',
      'my_home_line', 'my_total', 'snap_fair_home_line', 'snap_fair_total', 'snap_market_home_line', 'snap_gap_pts', 'snap_win_prob_home', 'snap_reliability_score', 'snap_model_version',
  ...
  function journalRowHTML(e) {
    ...
    return '<div class="edm-jr"><div class="edm-wr-h"><b>' + esc(s.away + ' @ ' + s.home) + '</b><span class="edm-dec ' + esc(e.decision) + '">' + esc(P.DECISION_LABEL[e.decision] || e.decision) + '</span>'
      ...
      + (hasNumber(e) ? myNumberRowHTML(e, s) : '')
      + '<div class="edm-grid">'
  ...
  function myNumberRowHTML(e, s) {
    var vc = P.numbersVsClose(e), bits = [];
    if (vc.close_home_line != null && vc.mine != null) bits.push('close ' + P.favText(s, vc.close_home_line) + ': yours ' + vc.mine.toFixed(1) + ' pts from it, EdgeDesk’s '
      + (vc.edgedesk != null ? vc.edgedesk.toFixed(1) + ' pts' : 'not on file'));
    if (vc.close_total != null && vc.mine_total != null) bits.push('closing total ' + vc.close_total + ': yours ' + vc.mine_total.toFixed(1) + ' pts from it'
      + (vc.edgedesk_total != null ? ', EdgeDesk’s ' + vc.edgedesk_total.toFixed(1) : ''));
    var pending = !bits.length && e.kickoff_at && Date.parse(e.kickoff_at) > Date.now();
    return '<div class="edm-mine"><i>Your number</i> <b>' + esc(myNumberText(e)) + '</b>'
      + (bits.length ? ' · ' + esc(bits.join(' · ')) : (pending ? ' <span class="edm-sub">· the close is recorded after kickoff</span>' : '')) + '</div>';
  }
  ...
  function qualityHTML() {
    ...
    return '<div class="edm-kpis">'
      ...
      + '</div>' + numbersQualityHTML();
  }
  function numbersQualityHTML() {
    var ns = P.numbersSummary(S.journal || []);
    if (!ns.n) return '';
    function d(x) { return x == null ? '—' : x.toFixed(1) + ' pts'; }
    return '<div class="edm-tbl"><div class="edm-tbl-h">Your numbers, EdgeDesk’s and the close</div><div class="edm-grid">'
      + '<div><i>Numbers saved</i>' + ns.n + ' (' + plural(ns.spreads, 'spread') + ', ' + plural(ns.totals, 'total') + ')</div>'
      + '<div><i>Average distance from EdgeDesk’s spread</i>' + d(ns.avg_apart_from_edgedesk) + '</div>'
      + '<div><i>With a close on file</i>' + ns.graded + '</div>'
      + '<div><i>Average distance from the close</i>' + (ns.graded ? 'yours ' + d(ns.avg_mine_from_close) + ' · EdgeDesk’s ' + d(ns.avg_edgedesk_from_close) : '—') + '</div>'
      + (ns.graded ? '<div class="wide"><i>Nearer the close</i>yours in ' + ns.mine_nearer + ' · EdgeDesk’s in ' + ns.edgedesk_nearer + ' · level in ' + ns.level + '</div>' : '')
      + '</div>' + (ns.sample_note ? '<div class="edm-note">' + esc(ns.sample_note) + '</div>' : '')
      + '<div class="edm-note">The close is the market’s last word, not the truth: this measures distance, and declares neither number right.</div></div>';
  }
  ...
  document.addEventListener('keydown', function (e) {
    ...
    ['edmJModal', 'edmHow', 'edmCModal', 'edmSModal', 'edmPersona'].forEach(function (id) { var h = $(id); if (h && h.classList.contains('on')) M.modalClose(id); });
  ...
  /* THE STEPS: 1 how you research (the persona), 2 leagues, 3 sportsbooks,
     4 what matters, 5 research alerts, 6 done. */
  var ONB = { persona: null, leagues: [], books: [], interests: [], alerts: null };
  var ONB_LAST = 6;
  /* start (or restart, from Settings) with what the account already holds */
  M.onbStart = function () {
    if (S.prefs) {
      ONB.persona = S.prefs.persona || null;
      ONB.leagues = (S.prefs.leagues || []).slice(); ONB.books = (S.prefs.books || []).slice(); ONB.interests = (S.prefs.interests || []).slice();
    }
    ONB.alerts = P.alertPrefs(S.alertPrefs);
    M.onboard(1);
  };
  M.onboard = function (step) {
    ...
    var dots = '<div class="edm-dots">' + [1, 2, 3, 4, 5, 6].map(function (i) { return '<i class="' + (i === step ? 'on' : (i < step ? 'done' : '')) + '"></i>'; }).join('') + '</div>';
    ...
    var body;
    if (step === 1) body = '<h3>What best describes how you research?</h3><p class="edm-sub">One answer. It only decides what your desk shows first — every tool stays one tap away.</p>'
      + chips(P.PERSONAS, ONB.persona ? [ONB.persona] : [], 'persona');
    else if (step === 2) body = '<h3>What do you research?</h3><p class="edm-sub">Your board, Top 5 and alerts start here. You can change this any time in Settings.</p>'
    ...
    else if (step === 3) body = '<h3>Which sportsbooks do you use?</h3><p class="edm-sub">Used to fill in your research journal. EdgeDesk never places a wager anywhere.</p>' + chips(P.BOOKS, ONB.books, 'books');
    else if (step === 4) body = '<h3>What matters most to you?</h3><p class="edm-sub">Pick any. It decides what the desk shows you first.</p>' + chips(P.INTERESTS, ONB.interests, 'interests');
    else if (step === 5) {
      var a = ONB.alerts;
      ...
      var pe = P.personaOf(ONB.persona);
      body = '<h3>Your research desk is ready</h3><ul class="edm-ul">'
        + (pe ? '<li><b>Arranged for how you research.</b> ' + esc(pe.note) + '</li>' : '')
        ...
        + '<li><b>Compare my number</b> sets your own fair line beside EdgeDesk’s and the market’s; <b>Log decision</b> saves your read with EdgeDesk’s numbers frozen, then grades it against the close.</li></ul>'
    ...
    host.innerHTML = '<div class="edm-modal" role="dialog" aria-modal="true" aria-label="Set up your research desk">' + dots + body
      ...
      + (step > 1 && step < ONB_LAST ? '<button class="edm-btn ghost" onclick="EDMine.onboard(' + (step - 1) + ')">Back</button>' : '')
      + (step < ONB_LAST ? '<button class="edm-btn" onclick="EDMine.onbNext(' + step + ')">Continue</button>' : '<button class="edm-btn" onclick="EDMine.onbFinish(\'completed\')">Go to my research desk</button>')
      + '<span class="edm-sp"></span><button class="edm-btn ghost" onclick="EDMine.onbFinish(\'skipped\')">Skip for now</button></div>'
      + '<div class="edm-foot">Step ' + step + ' of ' + ONB_LAST + ' · about a minute</div></div>';
    host.classList.add('on');
    if (step === 1) event('onboarding_start', {});
  };
  M.onbPick = function (group, key, btn) {
    if (group === 'persona') { ONB.persona = ONB.persona === key ? null : key; M.onboard(1); return; }
    var list = ONB[group];
    if (group === 'leagues' && key === 'both') { ONB.leagues = ONB.leagues.length === P.LEAGUES.length ? [] : P.LEAGUES.map(function (l) { return l.key; }); M.onboard(2); return; }
    if (group === 'interests' && key === 'all') { ONB.interests = ONB.interests.indexOf('all') >= 0 ? [] : P.INTERESTS.map(function (i) { return i.key; }); M.onboard(4); return; }
    var i = list.indexOf(key);
    if (i >= 0) list.splice(i, 1); else list.push(key);
    if (group === 'leagues') { M.onboard(2); return; }
  ...
  M.onbFinish = async function (status) {
    var row = { persona: ONB.persona || null, leagues: ONB.leagues, books: ONB.books, interests: ONB.interests, onboarding_status: status, timezone: (function () { try { return Intl.DateTimeFormat().resolvedOptions().timeZone || null; } catch (_) { return null; } })() };
    try {
    ...
    event(status === 'completed' ? 'onboarding_complete' : 'onboarding_skip', { leagues: ONB.leagues.join(','), persona: ONB.persona || '' });
    M.modalClose('edmOnb');
    if (status === 'completed') {
  ...
  M.settingsResearch = function () {
    ...
    var pe = P.personaOf(p.persona);
    return card('Research preferences', 'What your desk, Top 5 and alerts are built around. Stored on your account.',
      row('How you research', '<span class="set-val">' + esc(pe ? pe.label : 'not set') + '</span> <button class="btn ghost sm" onclick="EDMine.personaOpen()">' + (pe ? 'Change' : 'Choose') + '</button>',
        'Decides what your desk shows first. Nothing is hidden.')
      ...
      + '<div class="set-act"><button class="btn sm" onclick="EDMine.onbStart()">Edit preferences</button></div>')
  ...
  M.settingsPartner = function () {
    ...
    return card('Your partner link', 'Status: ' + esc(a.status) + ' · commission ' + Math.round(a.commission_rate * 100) + '%' + (set.commission_duration_months ? ' for ' + set.commission_duration_months + ' months' : '') + ' · ' + set.hold_days + '-day hold',
      ...
      + campaignsCardHTML(d.campaigns || []);
  };
  /* the creator's own campaigns: each code's terms, window and counts. The
     customer's discount is shown only as Stripe records it. */
  function campaignsCardHTML(list) {
    if (!list.length) return '';
    var X = window.EDPricing;
    return card('Your campaigns', 'Each campaign is its own code with its own terms, set by EdgeDesk. An account keeps the terms it was attributed on, even if the campaign changes later.',
      list.map(function (c) {
        var link = 'https://edgedesksports.com/?ref=' + encodeURIComponent(c.code);
        var terms = Math.round(c.commission_rate * 1000) / 10 + '% ' + (c.commission_type === 'one_time' ? 'of the first payment' : 'of each payment' + (c.commission_duration_months ? ' for ' + c.commission_duration_months + ' months' : ''));
        var disc = c.discount ? (X && X.discountLine ? X.discountLine(c.discount, c.code) : 'discount confirmed by Stripe') : (c.stripe_state === 'no_discount' ? 'no customer discount' : 'customer discount not yet confirmed by Stripe');
        var window_ = (c.starts_at ? 'from ' + when(c.starts_at) : '') + (c.expires_at ? ' to ' + when(c.expires_at) : '');
        return row(c.code + (c.name ? ' · ' + c.name : ''), '<span class="set-val">' + esc(c.in_force ? 'live' : (c.active ? 'not in force' : 'switched off')) + '</span> <button class="btn ghost sm" onclick="EDMine.copy(' + js(link) + ')">Copy link</button>',
          terms + ' · ' + disc + (window_ ? ' · ' + window_ : '') + ' · ' + (c.stats ? (c.stats.clicks || 0) + ' clicks, ' + (c.stats.signups || 0) + ' signups, ' + (c.stats.trials || 0) + ' trials, ' + (c.stats.paid_customers || 0) + ' paying' : ''));
      }).join(''));
  }
  ...
  /* ------------------------------------------------------------ acquisition
     Where this account came from (supabase/growth.sql): the visitor id and
     the touches the landing page kept (edgedesk_attribution, first touch;
     edgedesk_attribution_last). The server classifies them and freezes the
     first; it never touches the partner program's attribution. Once per
     account per device. */
  async function claimAcquisition() {
    try {
      var u = user(); if (!u || !u.id) return;
      if (localStorage.getItem('edgedesk_acq_claimed_v1') === u.id) return;
      function touch(x) {
        if (!x || typeof x !== 'object') return null;
        return { ref: x.ref || null, utm_source: x.utm_source || null, utm_medium: x.utm_medium || null, utm_campaign: x.utm_campaign || null,
          referrer_host: x.referrer || x.referrer_host || null, landing: x.landing || null, seen_at: x.seen_at || x.organic_first_seen_at || null };
      }
      var first = null, last = null;
      try { first = touch(JSON.parse(localStorage.getItem('edgedesk_attribution') || 'null')); } catch (_) { }
      try { last = touch(JSON.parse(localStorage.getItem('edgedesk_attribution_last') || 'null')); } catch (_) { }
      var v = localStorage.getItem('edgedesk_visitor');
      var r = await rpc('acq_claim', { p_visitor: P.validVisitor(v) ? v : null, p_first: first, p_last: last });
      if (r && r.ok) localStorage.setItem('edgedesk_acq_claimed_v1', u.id);
    } catch (_) { /* not installed yet, or offline: asked again on a later visit */ }
  }
  M.claimAcquisition = claimAcquisition;
