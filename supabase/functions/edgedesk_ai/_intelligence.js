...
(function (root, factory) {
  ...
  function researchPriority(o) {
    ...
    var coaching = o.coaching_research || null;
    if (coaching) {
      var shadow = coaching.shadow_reference || null;
      var shift = shadow ? Math.abs(num(shadow.home_margin_delta) || 0) : 0;
      if (shift >= 0.25) {
        add(2, 'Coaching / Staff has a pronounced calibrated research contrast in this matchup (' +
          shift.toFixed(2) + ' points at the unvalidated 1.0-point reference cap). It does not alter the official projection, but it is worth investigating.');
      } else if (shift >= 0.10) {
        add(1, 'Coaching / Staff has a measurable calibrated research contrast in this matchup (' +
          shift.toFixed(2) + ' points at the unvalidated 1.0-point reference cap). It does not alter the official projection.');
      } else if (coaching.research_read && coaching.research_read.state === 'RAW_COMPONENT_EVIDENCE') {
        add(1, 'Both teams have comparable raw Coaching / Staff evidence on at least one measured component. The league cohort is not yet sufficient for a calibrated shadow, so this raises research priority only.');
      }
    }
  ...
  function rankFootballCard(o) {
    ...
    games.forEach(function (g) {
      ...
      var pr = researchPriority({
        ...
        coaching_research: g.coaching_staff_research || null
      });
      rows.push({
        ...
        disagreement: dis,
        coaching_staff_research: g.coaching_staff_research || null,
