...
(function (root, factory) {
  ...
  /* ----------------------------------------------------------- availability */
  /* The injury report, when it IS on file. Shape (built by the app from the
     nflverse report the repo commits):
       { status:'ON_FILE'|'NOT_PUBLISHED'|'NO_REPORT', source, week,
         teams:{ away:{name, code, week, players:[{name, position, status, injury, practice}]},
                 home:{...} } }
     Copy only: it never touches a number or a verdict. */
  var STATUS_RANK = { out: 3, doubtful: 2, questionable: 1 };
  function statusWord(s) { s = String(s == null ? '' : s).toLowerCase(); return s === 'out' ? 'out' : s === 'doubtful' ? 'doubtful' : s === 'questionable' ? 'questionable' : null; }
  function availabilitySummary(av) {
    if (!av || av.status !== 'ON_FILE' || !av.teams) return null;
    var sides = [], listed = [], worst = 0;
    ['away', 'home'].forEach(function (k) {
      var tm = av.teams[k]; if (!tm) return;
      var ps = (tm.players || []).filter(function (p) { return statusWord(p.status); });
      var counts = { out: 0, doubtful: 0, questionable: 0 };
      ps.forEach(function (p) { var w = statusWord(p.status); counts[w]++; worst = Math.max(worst, STATUS_RANK[w]); listed.push({ team: tm.name || tm.code, name: p.name, position: p.position || null, status: w, injury: p.injury || null, practice: p.practice || null }); });
      var bits = [];
      ...
      sides.push(label + (tm.filed === false ? ': no report filed yet' : (bits.length ? ': ' + bits.join(', ') : ': nobody listed')));
    });
    listed.sort(function (a, b) { return STATUS_RANK[b.status] - STATUS_RANK[a.status] || a.name.localeCompare(b.name); });
    return { text: 'Injury report on file (' + (av.source || 'league report') + (av.week ? ', week ' + av.week : '') + '): ' + sides.join(' · '), sides: sides, listed: listed, worst: worst };
  }
  function normTeamName(s) { return normTeam(s); }
  function playerLine(p) { return p.name + (p.position ? ' (' + p.position + ')' : '') + ' is ' + p.status + (p.injury ? ', ' + String(p.injury).toLowerCase() : '') + (p.practice && /did not/i.test(p.practice) ? ', did not practice' : p.practice && /limited/i.test(p.practice) ? ', limited in practice' : '') + '.'; }
  ...
  function simpleFromPacket(p, ctx) {
    ...
    if (avail && avail.listed.length && (displayVerdict === 'BET' || displayVerdict === 'LEAN' || displayVerdict === 'WAIT')) {
      var pickTeam = (selectionRaw && game.home && normTeamName(selectionRaw) === normTeamName(game.home)) ? 'home' : (selectionRaw && game.away && normTeamName(selectionRaw) === normTeamName(game.away)) ? 'away' : null;
      var mine = avail.listed.filter(function (x) { return pickTeam && availability.teams[pickTeam] && x.team === (availability.teams[pickTeam].name || availability.teams[pickTeam].code) && x.status !== 'questionable'; });
      var theirs = avail.listed.filter(function (x) { return !mine.some(function (m) { return m === x; }) && x.status !== 'questionable'; });
      var lead = mine.length ? playerLine(mine[0]) + (mine.length > 1 ? ' ' + (mine.length - 1) + ' more on the same side listed.' : '') : (theirs.length ? 'For the other side, ' + playerLine(theirs[0]).charAt(0).toLowerCase() + playerLine(theirs[0]).slice(1) : '');
    ...
    if (avail) flags.push({ kind: 'AVAILABILITY', text: avail.text.replace(/^Injury report on file \([^)]*\): /, 'Injury report: '), severity: avail.worst >= 2 ? 'warn' : 'ok' });
    ...
    return {
      ...
      flags: flags,
      gaps: gaps,
      availability: availability ? { status: availability.status, source: availability.source || null, week: availability.week || null, summary: avail ? avail.text : null, listed: avail ? avail.listed.slice(0, 8) : [], sides: avail ? avail.sides : [], reason: availability.reason || null } : null,
  ...
  function cardHTML(s, opts) {
    ...
    if (!opts.compact) {
      h += '<div class="dcard-body">'
        ...
        + (s.availability && s.availability.listed && s.availability.listed.length ? '<div class="dcard-h">Injury report</div><ul class="dcard-avail">' + s.availability.listed.slice(0, 6).map(function (p) { return '<li><b>' + esc(p.name) + '</b>' + (p.position ? ' <span class="pos">' + esc(p.position) + '</span>' : '') + ' <span class="st ' + esc(p.status) + '">' + esc(p.status) + '</span>' + (p.injury ? ' · ' + esc(String(p.injury).toLowerCase()) : '') + (p.practice ? ' · ' + esc(String(p.practice).replace(/ in practice| participation/gi, '').toLowerCase()) : '') + ' <span class="tm">' + esc(p.team) + '</span></li>'; }).join('') + '</ul>' : '')
  ...
  return {
    ...
    availabilitySummary: availabilitySummary, playerLine: playerLine,
...
const BUILD = "edgedesk_ai-2026-09-02-r4-presentation";
...
Your job here is compression, not display. A person who knows nothing about betting must understand the answer in five seconds. Target an eighth-grade reading level. Short sentences. Short bullets. No unexplained acronyms. Never start with background — start with the decision.
...
2. The selection and the current price TOGETHER in one phrase ("Texas Tech -3.5 at -110"). The number is part of the recommendation. Never reduce it to "bet Texas Tech".
3. The playable limit, from the card's "good to" price only ("good to -115"). Never invent a threshold, and never state a line threshold — EdgeDesk owns price limits, not line limits.
...
Under 120 words in total. Sportsbook language: say "the sharper market" not "Pinnacle de-vig fair"; "good to" not "max playable"; "closing line value" not "CLV"; "some of the underlying data needs verification" not "data integrity warning". If the verdict is PASS, "PASS: EdgeDesk does not see enough value at the current price." followed by the one reason is a complete, successful answer — do not force action. If EVIDENCE INTEGRITY is FAIL, recommend nothing: say a data check failed and name it. If the price is stale, say the price needs refreshing before anyone acts. Plain prose and "- " bullets only; no headers.`,
...
The full ANSWER SHAPE and RESEARCH TRACE rules above apply unchanged. Precise terminology is welcome here; this is the layer sophisticated users open on purpose. The verdict, side, price and playable limit are still the engine's and are quoted exactly.`,
  PUBLISHER: `PRESENTATION MODE: PUBLISHER — neutral, polished, article-ready copy for a sports-media writer.
Write as a publication would: third person, neutral, confident only where the evidence is, short, specific, easy to quote. No first person, no "I", no "AI", no "our algorithm", no chat tone, no headers, no bullets — plain paragraphs a writer can lift straight into a story. Never use: lock, guaranteed, hammer, can't miss, free money, AI says. Prefer: "EdgeDesk's current research favors…", "The current number remains playable through…", "The strongest reason is…", "The biggest concern is…", "At a worse number, EdgeDesk would pass." The decision, selection, price and playable limit are the engine's and are quoted exactly from DECISION CARD FACTS. If there is no bet, say so plainly and explain what EdgeDesk would need to see. Under 200 words.`,
};
