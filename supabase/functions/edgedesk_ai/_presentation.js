...
(function (root, factory) {
  ...
  function verdictPlain(o) {
    ...
    /* A MISSING PRICE DISABLES THE COMPARISON, NOT THE RESEARCH. This
       sentence used to read "there is nothing to judge", which is false of
       every football game EdgeDesk rates: what it cannot do without a
       posted price is compare its own number with someone else's. */
    else if (o.no_price) answer = 'No sportsbook price is on file yet, so there is no market number to compare EdgeDesk\u2019s own against.';
  ...
  var PRESETS = {
    ...
    RANKINGS: { title: 'EdgeDesk College Football Rankings', kicker: null }
  };
  ...
  /* THE CALL LINE, AND WHAT IT SAYS WHEN THERE IS NO BET. On a game no book
     has posted there is no selection to name, and concatenating one printed
     the literal word "null" beside the status — "WAIT — null" — in both the
     plain-text brief and the CMS paste. The status stands on its own. */
  function callLine(b, pl) {
    if (!pl) return (b.call && b.call.text) || 'No decision on file.';
    var bet = txt(pl.bet);
    if (!bet) return pl.status + ' — no priced selection on file for this game';
    return pl.status + ' — ' + bet + (pl.price ? ' (' + pl.price + ')' : '');
  }
  ...
  function publisher(simple, ctx) {
    ...
    /* "below" is a promise about the page. It is only made when the research
    ...
    /* A PRICE THAT IS NOT ON FILE IS NOT A PRICE. Concatenating ' at ' +
       s.odds unguarded published the literal word "null" in the lede of every
       unpriced game — "EdgeDesk is not ready to call this bet at null". The
       clause is dropped instead. The absent price is then STATED, in the
       branch below, rather than papered over: a brief on a game no book has
       posted is a research brief, and it says so in its first sentence. */
    var atOdds = s.odds ? ' at ' + s.odds : '';
    var lede;
    ...
    else if (v === 'BET') lede = 'EdgeDesk’s research favors ' + betMid + atOdds + '.' + (goodTo.limit_odds ? ' The price stays good enough down to ' + goodTo.limit_odds + '.' : '');
    else if (v === 'LEAN') lede = 'EdgeDesk leans toward ' + betMid + atOdds + ', with caveats.' + (goodTo.limit_odds ? ' The price stays good enough down to ' + goodTo.limit_odds + '.' : '');
    /* A BRIEF IS NOT A BET SLIP. The old lede on an unpriced game read
       "EdgeDesk has no bet to call", which made the absence of a sportsbook
       number the subject of the whole page — and on a college game that is
       simply wrong. EdgeDesk has a projection, two team ratings, twenty
       ranked components under them, a position-group comparison and an
       account of what it could not measure, none of which needs a book.
       What a missing price removes is ONE section: the comparison. */
    else if (v === 'WAIT') lede = s.odds
      ? 'EdgeDesk is not ready to call ' + betMid + ' at ' + s.odds + '. ' + (pl.answer || 'Something still has to be confirmed.')
      : (ctx.research_follows
        ? 'No sportsbook has posted a price on this game yet, so there is no model-versus-market comparison to make. EdgeDesk\u2019s own research on the matchup stands unchanged — the projection, the team and roster comparison behind it, the matchups that decide it, and what is still unknown.'
        : 'No sportsbook has posted a price on this game yet, so there is no market number for EdgeDesk to compare its own against. ' + (pl.answer || 'Nothing is published in its place.'));
  ...
  /* ======================================================================
     THE RESEARCH THAT IS NOT A PRICE

     A brief used to be a market object: a verdict, a number, a book. On a
     game no sportsbook had posted, every one of those fields was empty and
     the page said so six different ways — and printed the word "null" where
     the price belonged. But EdgeDesk always had something to say about that
     game. It had a projection, two team ratings, a unit-by-unit read and an
     honest account of what it could not measure yet.

     These two normalisers let a board hand that read in. THEY DERIVE
     NOTHING. Every number arrives already formatted by the one canonical
     build; this decides only where it sits on the page and refuses to print
     anything it was not given. A field that is absent stays absent — "not
     measured" and "measured zero" are different sentences and neither is
     allowed to impersonate the other.
     ====================================================================== */
  function txt(v) {
    if (v == null) return null;
    var t = String(v).trim();
    /* A stringified null is the bug this whole block exists to stop. */
    return (t && t !== 'null' && t !== 'undefined' && t !== 'NaN') ? t : null;
  }
  function txtList(a, cap) {
    return (Array.isArray(a) ? a : []).map(txt).filter(Boolean).slice(0, cap || 12);
  }
  function sectionList(a, cap) {
    return (Array.isArray(a) ? a : []).map(function (x) {
      var h = txt(x && x.h), b = txt(x && x.p);
      return (h || b) ? { h: h, p: b } : null;
    }).filter(Boolean).slice(0, cap || 10);
  }
  /* ═══ THE MATCHUP RESEARCH, NORMALISED ═══════════════════════════════════
     A GAME BRIEF IS A RESEARCH DOCUMENT, NOT A MARKET OBJECT. Everything
     below travels on `research` and renders on its own: the projection, the
     two team ratings and every ranked component under them, where each side
     has the edge, the unit-versus-unit matchups, how the two rosters are
     built, the case for each team, and what EdgeDesk could not measure. The
     MARKET is one section of that document — `research.market` — and it is
     allowed to be absent. A brief on a game no book has posted is a full
     brief with one section that says no price is joined yet.

     NOTHING HERE COMPUTES. Every field is carried from the builder that owns
     it, put through txt() so a stringified null can never reach a page, and
     laid out. A payload from before these sections existed simply has none
     of them and renders exactly as it always did. */
  function cellOf(c) {
    if (!c) return { v: null, rank: null, note: null };
    return { v: txt(c.v), rank: txt(c.rank), note: txt(c.note) };
  }
  function edgeOf(e) {
    if (!e || (e.side !== 'a' && e.side !== 'h' && e.side !== 'even')) return null;
    return { side: e.side, gap: num(e.gap), rank_gap: num(e.rank_gap) };
  }
  function cmpRows(a, cap) {
    return (Array.isArray(a) ? a : []).map(function (x) {
      var k = txt(x && x.k);
      if (!k) return null;
      var A = cellOf(x.a), H = cellOf(x.h);
      if (A.v == null && H.v == null && !A.note && !H.note) return null;
      return { k: k, note: txt(x.note), a: A, h: H, edge: edgeOf(x.edge) };
    }).filter(Boolean).slice(0, cap || 30);
  }
  function advList(a, cap) {
    return (Array.isArray(a) ? a : []).map(function (x) {
      var k = txt(x && x.k), t = txt(x && x.text);
      if (!k || !t) return null;
      return { k: k, text: t, lead: txt(x.lead), trail: txt(x.trail), rank_gap: num(x.rank_gap) };
    }).filter(Boolean).slice(0, cap || 5);
  }
  /* THE PROJECTION RENDERS ON ITS OWN TERMS. It is published when the model
     produced a number, and it says so plainly when it did not. Neither
     branch consults a book: `market` is a different field. */
  function projectionOf(p) {
    if (!p) return null;
    if (!p.priced) {
      return { priced: false, absent_reason: txt(p.absent_reason), note: txt(p.note) };
    }
    var sc = null;
    if (p.score && p.score.home && p.score.away && txt(p.score.home.points) != null && txt(p.score.away.points) != null) {
      sc = { away: { team: txt(p.score.away.team) || 'Away', points: txt(p.score.away.points) },
        home: { team: txt(p.score.home.team) || 'Home', points: txt(p.score.home.points) },
        note: txt(p.score.note) };
    }
    var wp = null;
    if (p.win_prob && num(p.win_prob.home_pct) != null && num(p.win_prob.away_pct) != null) {
      wp = { home: txt(p.win_prob.home) || 'Home', away: txt(p.win_prob.away) || 'Away',
        home_pct: num(p.win_prob.home_pct), away_pct: num(p.win_prob.away_pct),
        /* the BOUNDED text, carried so every consumer prints the same thing
           and none of them re-rounds a probability back across the boundary.
           100% and 0% are the two values a continuous margin distribution
           cannot mean, and a card that printed them was telling a reader
           something the model never said. */
        home_text: txt(p.win_prob.home_text), away_text: txt(p.win_prob.away_text),
        bounded: p.win_prob.bounded === true, bounded_note: txt(p.win_prob.bounded_note),
        favourite: txt(p.win_prob.favourite), favourite_pct: num(p.win_prob.favourite_pct),
        favourite_text: txt(p.win_prob.favourite_text) };
    }
    var rg = null;
    if (p.outcome_range && txt(p.outcome_range.p10) && txt(p.outcome_range.p90)) {
      rg = { basis_team: txt(p.outcome_range.basis_team), p10: txt(p.outcome_range.p10),
        median: txt(p.outcome_range.median), p90: txt(p.outcome_range.p90),
        sigma: txt(p.outcome_range.sigma), volatility: txt(p.outcome_range.volatility),
  ...
  /* THE MARKET SECTION. Present or absent, it is a SECTION — never the
     premise of the page and never a reason to stop publishing research. */
  function marketOf(m) {
    if (!m) return null;
    if (!m.available) {
  ...
  function researchBlock(r) {
    if (!r) return null;
    var score = null;
    if (r.score && r.score.home && r.score.away && txt(r.score.home.points) != null && txt(r.score.away.points) != null) {
      score = {
        away: { team: txt(r.score.away.team) || 'Away', points: txt(r.score.away.points) },
        home: { team: txt(r.score.home.team) || 'Home', points: txt(r.score.home.points) },
        note: txt(r.score.note)
      };
    }
    var table = null;
    if (r.table && Array.isArray(r.table.rows)) {
      var rows = r.table.rows.map(function (x) {
        var k = txt(x && x.k);
        if (!k) return null;
        return { k: k, a: txt(x.a), h: txt(x.h), note: txt(x.note) };
      }).filter(Boolean);
      if (rows.length) {
        var c = r.table.cols || [];
        table = { cols: [txt(c[0]) || 'Away', txt(c[1]) || 'Home'], rows: rows.slice(0, 30) };
      }
    }
    var game = null;
    if (r.game && txt(r.game.title)) {
      var meta = [txt(r.game.when), txt(r.game.venue), r.game.neutral ? 'neutral site' : null,
        txt(r.game.conference_line), txt(r.game.week), txt(r.game.season)].filter(Boolean).join(' · ');
      game = { title: txt(r.game.title), meta: meta || null,
        away: txt(r.game.away), home: txt(r.game.home) };
    }
    var compare = null;
    if (r.compare && Array.isArray(r.compare.groups)) {
      var groups = r.compare.groups.map(function (g) {
        var rows = cmpRows(g && g.rows, 30);
        var t = txt(g && g.title);
        return rows.length ? { title: t || 'Ratings', rows: rows } : null;
      }).filter(Boolean).slice(0, 8);
      var cc = r.compare.cols || [];
      if (groups.length) compare = { cols: [txt(cc[0]) || 'Away', txt(cc[1]) || 'Home'], groups: groups,
        one_sided_team: txt(r.compare.one_sided_team), one_sided_note: txt(r.compare.one_sided_note) };
    }
    var drivers = null;
    if (r.drivers) {
      var drows = (Array.isArray(r.drivers.rows) ? r.drivers.rows : []).map(function (d) {
        var t = txt(d && d.text);
    ...
    var advantages = null;
    if (r.advantages) {
      advantages = { away: advList(r.advantages.away, 5), home: advList(r.advantages.home, 5),
        measured: advList(r.advantages.measured, 5), measured_note: txt(r.advantages.measured_note),
        unproven: txtList(r.advantages.unproven, 4), note: txt(r.advantages.note) };
    }
    var matchups = (Array.isArray(r.matchups) ? r.matchups : []).map(function (m) {
      var t = txt(m && m.title), rd = txt(m && m.read);
      if (!t || !rd) return null;
      return { title: t, read: rd, complete: !!m.complete,
        att: { team: txt(m.att && m.att.team) || '', label: txt(m.att && m.att.label) || '', v: txt(m.att && m.att.v), rank: txt(m.att && m.att.rank) },
        def: { team: txt(m.def && m.def.team) || '', label: txt(m.def && m.def.label) || '', v: txt(m.def && m.def.v), rank: txt(m.def && m.def.rank) } };
    }).filter(Boolean).slice(0, 14);
    var matchupsNote = txt(r.matchups && r.matchups.note);
    var roster = null;
    if (r.roster && Array.isArray(r.roster.rows)) {
      var rrows = r.roster.rows.map(function (x) {
        var k = txt(x && x.k);
        if (!k) return null;
        var a = txt(x.a), hh = txt(x.h);
        return (a == null && hh == null) ? null : { k: k, note: txt(x.note), a: a, h: hh };
      }).filter(Boolean).slice(0, 20);
      var rc = r.roster.cols || [];
      if (rrows.length) roster = { cols: [txt(rc[0]) || 'Away', txt(rc[1]) || 'Home'], rows: rrows,
        highlights: (Array.isArray(r.roster.highlights) ? r.roster.highlights : []).map(function (x) {
          var t = txt(x && x.text); return t ? { k: txt(x.k), lead: txt(x.lead), text: t } : null;
    ...
    if (r.cases) {
      function side(c) {
        if (!c) return null;
        var b = txtList(c.bullets, 5);
        var head = txt(c.heading);
        return (head || b.length) ? { team: txt(c.team), heading: head || 'The case', bullets: b } : null;
      }
      var fv = side(r.cases.favourite), dg = side(r.cases.underdog);
      if (fv || dg) cases = { favourite: fv, underdog: dg, note: txt(r.cases.note) };
    }
    var uncertainty = null;
    if (r.uncertainty) {
      var items = (Array.isArray(r.uncertainty.items) ? r.uncertainty.items : []).map(function (i) {
        var t = txt(i && i.text);
        if (!t) return null;
        var sev = txt(i.sev);
        sev = (sev === 'HIGH' || sev === 'MEDIUM' || sev === 'LOW') ? sev : 'MEDIUM';
        return { sev: sev, label: txt(i.label) || 'uncertainty', text: t, widening: txt(i.widening) };
      }).filter(Boolean).slice(0, 14);
      var um = (Array.isArray(r.uncertainty.unmeasured) ? r.uncertainty.unmeasured : []).map(function (u) {
        var it = txt(u && u.item);
        return it ? { item: it, why: txt(u.why) } : null;
      }).filter(Boolean).slice(0, 12);
      if (items.length || um.length) uncertainty = { items: items, unmeasured: um, note: txt(r.uncertainty.note) };
    }
    var out = {
      kind: txt(r.kind),
      game: game,
      lede: txtList(r.lede, 4),
      ...
      market: marketOf(r.market),
      table: table,
      notes: txtList(r.notes, 6),
      missing: txtList(r.missing, 10),
      priced_label: txt(r.priced_label),
      context_label: txt(r.context_label),
      source: txt(r.source)
    };
  ...
  function rankingsBlock(r) {
    if (!r) return null;
    var rows = (Array.isArray(r.rows) ? r.rows : []).map(function (x) {
      var team = txt(x && x.team);
      if (!team) return null;
      return {
        rank: txt(x.rank), team: team, conf: txt(x.conf),
        delta: txt(x.delta), delta_dir: x.delta_dir === 'up' ? 'up' : x.delta_dir === 'down' ? 'down' : null,
        cells: (Array.isArray(x.cells) ? x.cells : []).map(function (c) {
          return { v: txt(c && c.v), rank: txt(c && c.rank) };
        }).slice(0, 10)
      };
    }).filter(Boolean);
    var out = {
      headline: txt(r.headline), subhead: txt(r.subhead),
      columns: txtList(r.columns, 12),
      rows: rows.slice(0, 40),
      method: sectionList(r.method, 8),
      spotlights: sectionList(r.spotlights, 8),
      movers: sectionList(r.movers, 8),
      missing: txtList(r.missing, 12),
      footer: txtList(r.footer, 6),
      source: txt(r.source)
    };
    return (out.rows.length || out.method.length || out.spotlights.length) ? out : null;
  }
  /* ---- and the way each one reads on the page --------------------------- */
  function resSec(title, note) {
    return '<div class="edb-h edb-sec">' + esc(title) + '</div>'
      + (note ? '<p class="edb-secnote">' + esc(note) + '</p>' : '');
  }
  function resCell(c) {
    if (!c || c.v == null) {
      return '<td class="edb-c"><span class="edb-nodata">' + esc(c && c.note ? 'unavailable' : 'not measured') + '</span>'
        + (c && c.note ? '<span class="n">' + esc(c.note) + '</span>' : '') + '</td>';
    }
    return '<td class="edb-c"><b>' + esc(c.v) + '</b>'
      + (c.rank ? '<span class="n">' + esc(c.rank) + '</span>' : (c.note ? '<span class="n">' + esc(c.note) + '</span>' : '')) + '</td>';
  }
  function resCmpRow(x) {
    var lead = x.edge && x.edge.side !== 'even' ? x.edge.side : null;
    function cell(side) {
      var h = resCell(side === 'a' ? x.a : x.h);
      if (lead === side) h = h.replace('<td class="edb-c"',
        '<td class="edb-c adv" title="' + esc('EdgeDesk has this side ahead in ' + x.k.toLowerCase()
          + (x.edge.rank_gap != null ? ' by ' + x.edge.rank_gap + ' places nationally' : '')) + '"');
      return h;
    }
  ...
  function projectionHTML(p, gameTitle) {
    if (!p) return '';
    if (!p.priced) {
      return '<div class="edb-proj edb-unpriced"><div class="k">EdgeDesk projection</div>'
        + '<p class="edb-projnote">EdgeDesk has not priced this matchup yet'
        + (p.absent_reason ? ' — ' + esc(p.absent_reason) : '') + '.</p>'
        + (p.note ? '<p class="edb-projnote">' + esc(p.note) + '</p>' : '') + '</div>';
    }
    var h = '<div class="edb-proj"><div class="k">EdgeDesk fair spread</div>'
      + '<div class="edb-fair">' + esc(p.fair_spread_text || '') + '</div>'
      + '<div class="edb-projgrid">';
    function tile(k, v, sub, cls) {
      return '<div class="edb-t' + (cls ? ' ' + cls : '') + '"><div class="tk">' + esc(k) + '</div>'
        + '<div class="tv">' + v + '</div>' + (sub ? '<div class="ts">' + esc(sub) + '</div>' : '') + '</div>';
    }
    if (p.score) {
      h += tile('Projected score',
        '<span class="edb-sc"><i>' + esc(p.score.away.team) + '</i><b>' + esc(p.score.away.points) + '</b></span>'
        + '<span class="edb-sc"><i>' + esc(p.score.home.team) + '</i><b>' + esc(p.score.home.points) + '</b></span>',
        p.total ? 'from the model’s own margin and a ' + p.total + ' total' : null, 'wide');
    } else {
      h += tile('Projected score', '<span class="edb-nodata">not published</span>', p.score_absent_reason, 'wide');
    }
    ...
    if (p.win_prob) h += tile('Win probability',
      esc(p.win_prob.home) + ' ' + (p.win_prob.home_text || (p.win_prob.home_pct + '%')),
      p.win_prob.away + ' ' + (p.win_prob.away_text || (p.win_prob.away_pct + '%'))
        + (p.win_prob.bounded ? ' \u00b7 bounded: a projection cannot mean 100% or 0%' : ''));
    ...
    /* the score tile already carries the absent reason as its subtitle, so
       only the note that has nowhere else to go is repeated here */
    if (p.score && p.score.note) h += '<p class="edb-projnote">' + esc(p.score.note) + '</p>';
    if (p.outcome_range && p.outcome_range.volatility_discriminates === false)
      h += '<p class="edb-projnote">The volatility index for this game does not discriminate — it cannot tell this matchup apart from the field, so read it as no information rather than as calm.</p>';
    if (p.outcome_range && p.outcome_range.note) h += '<p class="edb-projnote">' + esc(p.outcome_range.note) + '</p>';
    if (p.validation) h += '<p class="edb-projnote">' + esc(p.validation) + (p.engine ? ' ' + esc(p.engine) + '.' : '') + '</p>';
    void gameTitle;
    return h + '</div>';
  }
  /* WHY THERE IS NO VERDICT, said once and said accurately. A verdict needs a
     CAPTURED PRICE on a specific selection. The research section may still
     carry a reference line — a book consensus joined to the game — and a
     paragraph that flatly said "no book has posted this game" contradicted
     the market table printed directly above it. */
  function noVerdictSentence(res) {
    var ref = res && res.market && res.market.available ? res.market : null;
    return 'EdgeDesk’s verdicts compare a captured price on a specific selection against its own number. '
      + (ref
        ? ('No such price is on file for this game — the ' + (ref.book || 'reference')
          + ' line above is a book number joined to the matchup, not a quote EdgeDesk can grade a bet against. ')
        : 'No book has posted this game, so there is no price to compare. ')
      + 'So there is no verdict on it — not a lean, not a pass. That disables the verdict and nothing else: the '
      + 'research above is EdgeDesk’s current view of the matchup, and the moment a price is captured this brief '
      + 'carries the comparison too.';
  }
  function marketHTML(mk) {
    ...
    function box(k, v, sub) {
      return '<div class="edb-mb"><div class="k">' + esc(k) + '</div><div class="v">' + esc(v) + '</div>'
  ...
  function researchHTML(res) {
    if (!res) return '';
    var h = '<section class="edb-res">';
    if (res.game) {
      h += '<div class="edb-gh"><div class="t">' + esc(res.game.title || '') + '</div>'
        + (res.game.meta ? '<div class="m">' + esc(res.game.meta) + '</div>' : '') + '</div>';
    }
    h += '<div class="edb-h edb-sec">The EdgeDesk research</div>';
    if (res.lede && res.lede.length) h += res.lede.map(function (s) { return '<p class="edb-lede">' + esc(s) + '</p>'; }).join('');
    else if (res.headline) h += '<p class="edb-lede">' + esc(res.headline) + '</p>';
    /* ---- 1. the projection. Never gated on a book. ---- */
    if (res.projection) {
      h += resSec('EdgeDesk projection', res.priced_label);
      h += projectionHTML(res.projection, res.game && res.game.title);
    } else if (res.score) {
      /* a payload published before the projection block existed */
      h += '<div class="edb-proj"><div class="k">Projected score</div><div class="edb-projline">'
        + '<span class="edb-sc"><i>' + esc(res.score.away.team) + '</i><b>' + esc(res.score.away.points) + '</b></span>'
    ...
    if (res.drivers) {
      ...
      if (res.drivers.rows.length) {
        var mx = 0;
        res.drivers.rows.forEach(function (d) { mx = Math.max(mx, Math.abs(d.points_n == null ? 0 : d.points_n)); });
        h += '<div class="edb-drv">' + res.drivers.rows.map(function (d) {
          var pn = d.points_n == null ? 0 : d.points_n;
          return '<div class="edb-d"><span class="p ' + (pn >= 0 ? 'pos' : 'neg') + '">' + esc(d.points || '') + '</span>'
            + '<span class="b"><i style="width:' + (mx ? Math.round(Math.abs(pn) / mx * 100) : 0) + '%"></i></span>'
    ...
    if (res.compare && res.compare.groups.length) {
      h += resSec('Team research head-to-head', res.context_label);
      if (res.compare.one_sided_note) h += '<p class="edb-secnote edb-warnnote">' + esc(res.compare.one_sided_note) + '</p>';
      h += '<div class="edb-tblwrap"><table class="edb-tbl edb-cmptbl"><thead><tr><th></th><th>'
        + esc(res.compare.cols[0]) + '</th><th>' + esc(res.compare.cols[1]) + '</th></tr></thead><tbody>';
      res.compare.groups.forEach(function (g) {
        h += '<tr class="edb-grp"><th scope="rowgroup" colspan="3">' + esc(g.title) + '</th></tr>';
        g.rows.forEach(function (x) { h += resCmpRow(x); });
      });
      h += '</tbody></table></div>';
    } else if (res.table) {
      h += '<div class="edb-tblwrap"><table class="edb-tbl"><thead><tr><th></th><th>' + esc(res.table.cols[0]) + '</th><th>' + esc(res.table.cols[1]) + '</th></tr></thead><tbody>';
      res.table.rows.forEach(function (x) {
        h += '<tr><th scope="row">' + esc(x.k) + (x.note ? '<span class="n">' + esc(x.note) + '</span>' : '') + '</th>'
          + '<td>' + (x.a ? esc(x.a) : '<span class="edb-nodata">not measured</span>') + '</td>'
          + '<td>' + (x.h ? esc(x.h) : '<span class="edb-nodata">not measured</span>') + '</td></tr>';
      });
      h += '</tbody></table></div>';
    }
    /* ---- 4. where each team has the edge ---- */
    if (res.advantages && (res.advantages.away.length || res.advantages.home.length)) {
      h += resSec('Where each team has the edge', res.advantages.note);
      h += '<div class="edb-two">';
      [[res.compare ? res.compare.cols[0] : 'Away', res.advantages.away],
       [res.compare ? res.compare.cols[1] : 'Home', res.advantages.home]].forEach(function (pair) {
        h += '<div><div class="edb-caseh">' + esc(pair[0]) + ' advantages</div>'
          + (pair[1].length
            ? '<ul class="edb-why">' + pair[1].map(function (a) { return '<li><b>' + esc(a.k) + '</b><br>' + esc(a.text) + '</li>'; }).join('') + '</ul>'
            : '<p class="edb-secnote">No category where both teams carry a national rank favours ' + esc(pair[0]) + '.</p>')
          + '</div>';
      });
      h += '</div>';
    } else if (res.advantages && (res.advantages.measured.length || res.advantages.unproven.length || res.advantages.note)) {
      /* NEITHER SIDE CAN BE SHOWN AHEAD, so neither is. What is measurable is
         printed as measurement and what is unknown is printed as uncertainty
         — an empty heading would tell the writer nothing. */
      h += resSec('Where each team has the edge', res.advantages.note);
      if (res.advantages.measured.length) {
        h += '<div class="edb-caseh">What EdgeDesk can measure</div>'
          + (res.advantages.measured_note ? '<p class="edb-secnote">' + esc(res.advantages.measured_note) + '</p>' : '')
          + '<ul class="edb-why">'
          + res.advantages.measured.map(function (a) { return '<li><b>' + esc(a.k) + '</b> — ' + esc(a.text) + '</li>'; }).join('')
          + '</ul>';
      }
      if (res.advantages.unproven.length) {
        h += '<div class="edb-caseh">Why the size of the gap is not established</div><ul class="edb-why edb-miss">'
          + res.advantages.unproven.map(function (n) { return '<li>' + esc(n) + '</li>'; }).join('') + '</ul>';
      }
    }
    /* ---- 5. the matchups that matter ---- */
    if (res.matchups && res.matchups.length) {
      h += resSec('The matchups that matter', res.matchups_note);
      h += '<div class="edb-mus">' + res.matchups.map(function (m) {
        return '<div class="edb-mu' + (m.complete ? '' : ' partial') + '">'
          + '<div class="mh">' + esc(m.title) + '</div>'
          + '<div class="mr"><span>' + esc(m.att.team) + ' ' + esc(m.att.label) + '</span><b>'
          + (m.att.v ? esc(m.att.v) + (m.att.rank ? ' · ' + esc(m.att.rank) : '') : '<span class="edb-nodata">unavailable</span>') + '</b></div>'
          + '<div class="mr"><span>' + esc(m.def.team) + ' ' + esc(m.def.label) + '</span><b>'
          + (m.def.v ? esc(m.def.v) + (m.def.rank ? ' · ' + esc(m.def.rank) : '') : '<span class="edb-nodata">unavailable</span>') + '</b></div>'
    ...
    if (res.roster && res.roster.rows.length) {
      ...
      if (res.roster.highlights.length) {
        h += '<ul class="edb-why">' + res.roster.highlights.map(function (x) { return '<li>' + esc(x.text) + '</li>'; }).join('') + '</ul>';
      }
      h += '<div class="edb-tblwrap"><table class="edb-tbl"><thead><tr><th></th><th>'
        + esc(res.roster.cols[0]) + '</th><th>' + esc(res.roster.cols[1]) + '</th></tr></thead><tbody>';
      res.roster.rows.forEach(function (x) {
        h += '<tr><th scope="row">' + esc(x.k) + (x.note ? '<span class="n">' + esc(x.note) + '</span>' : '') + '</th>'
          + '<td>' + (x.a ? esc(x.a) : '<span class="edb-nodata">not measured</span>') + '</td>'
          + '<td>' + (x.h ? esc(x.h) : '<span class="edb-nodata">not measured</span>') + '</td></tr>';
      });
    ...
    /* ---- 7. the case for each team ---- */
    if (res.cases) {
      h += resSec('The case for each team', res.cases.note);
      h += '<div class="edb-two">';
      [res.cases.favourite, res.cases.underdog].forEach(function (c) {
        if (!c) return;
        h += '<div><div class="edb-caseh">' + esc(c.heading) + '</div>'
          + (c.bullets.length ? '<ul class="edb-why">' + c.bullets.map(function (b) { return '<li>' + esc(b) + '</li>'; }).join('') + '</ul>'
            : '<p class="edb-secnote">Nothing measurable was published on this side.</p>') + '</div>';
      });
      h += '</div>';
    }
    /* ---- 8. why the number could be wrong ---- */
    if (res.uncertainty && (res.uncertainty.items.length || res.uncertainty.unmeasured.length)) {
      h += resSec('Why the number could be wrong', res.uncertainty.note);
      if (res.uncertainty.items.length) {
        h += '<div class="edb-rsk">' + res.uncertainty.items.map(function (i) {
          return '<div class="edb-r"><span class="s ' + esc(String(i.sev).toLowerCase()) + '">' + esc(i.sev) + '</span>'
            + '<span class="t"><b>' + esc(i.label) + '</b>'
            + (i.widening ? ' <span class="n">widens the range ' + esc(i.widening) + '</span>' : ' <span class="n">not quantified</span>')
            + '<br>' + esc(i.text) + '</span></div>';
        }).join('') + '</div>';
      }
      if (res.uncertainty.unmeasured.length) {
        h += '<div class="edb-h">Not measured at all</div><ul class="edb-why edb-miss">'
          + res.uncertainty.unmeasured.map(function (u) {
            return '<li><b>' + esc(u.item) + '</b>' + (u.why ? ' — ' + esc(u.why) : '') + '</li>';
          }).join('') + '</ul>';
      }
    }
    /* ---- the prose notes the first version of this brief carried. They
         restate the projection in sentences, so a payload that also carries
         the projection SECTION prints them once, not twice. An older payload
         has only the notes, and keeps them. ---- */
    if (res.notes.length && !res.projection) h += '<ul class="edb-why">' + res.notes.map(function (n) { return '<li>' + esc(n) + '</li>'; }).join('') + '</ul>';
    /* ---- 9. THE MARKET. Last, and optional. ---- */
    h += marketHTML(res.market);
    /* ---- 10. the research state, AFTER the research ---- */
    if (res.state) {
      h += resSec('Research state')
        + '<p class="edb-resstate"><b>' + esc(res.state.label) + '</b>'
        + (res.state.note ? ' — ' + esc(res.state.note) : '') + '</p>'
        + (res.state.scope ? '<p class="edb-secnote">' + esc(res.state.scope) + '</p>' : '');
    }
    if (res.missing.length) {
      h += '<div class="edb-h">What EdgeDesk could not measure</div><ul class="edb-why edb-miss">'
        + res.missing.map(function (n) { return '<li>' + esc(n) + '</li>'; }).join('') + '</ul>';
    }
    if (res.source) h += '<p class="edb-ressrc">' + esc(res.source) + '</p>';
    return h + '</section>';
  }
  function rankingsHTML(rk) {
    if (!rk) return '';
    var h = '<section class="edb-rk">';
    if (rk.headline) h += '<p class="edb-lede">' + esc(rk.headline) + '</p>';
    if (rk.subhead) h += '<p class="edb-rksub">' + esc(rk.subhead) + '</p>';
    if (rk.rows.length) {
      h += '<div class="edb-tblwrap"><table class="edb-tbl edb-rktbl"><thead><tr>'
        + rk.columns.map(function (c, i) { return '<th' + (i ? ' class="num"' : '') + '>' + esc(c) + '</th>'; }).join('')
        + '</tr></thead><tbody>';
      rk.rows.forEach(function (r) {
        h += '<tr><td class="rk">' + esc(r.rank || '') + '</td>'
          + '<td class="tm"><b>' + esc(r.team) + '</b>' + (r.conf ? '<span class="n">' + esc(r.conf) + '</span>' : '') + '</td>';
        r.cells.forEach(function (c) {
          h += '<td class="num">' + (c.v ? esc(c.v) : '<span class="edb-nodata">—</span>')
            + (c.rank ? '<span class="n">' + esc(c.rank) + '</span>' : '') + '</td>';
        });
        h += '<td class="num mv' + (r.delta_dir ? ' ' + r.delta_dir : '') + '">' + (r.delta ? esc(r.delta) : '') + '</td></tr>';
      });
      h += '</tbody></table></div>';
    }
    function secs(list, heading) {
      if (!list.length) return '';
      var out = heading ? '<div class="edb-h edb-sec">' + esc(heading) + '</div>' : '';
      list.forEach(function (x) {
        out += '<div class="edb-rksec">' + (x.h ? '<h3>' + esc(x.h) + '</h3>' : '') + (x.p ? '<p>' + esc(x.p) + '</p>' : '') + '</div>';
      });
      return out;
    }
    h += secs(rk.method, 'How EdgeDesk ranks teams');
    h += secs(rk.movers, 'What moved this week');
    h += secs(rk.spotlights, null);
    if (rk.missing.length) {
      h += '<div class="edb-h edb-sec">What is missing, and why</div><ul class="edb-why edb-miss">'
        + rk.missing.map(function (n) { return '<li>' + esc(n) + '</li>'; }).join('') + '</ul>';
    }
    if (rk.footer.length) h += '<div class="edb-rkfoot">' + rk.footer.map(function (n) { return '<p>' + esc(n) + '</p>'; }).join('') + '</div>';
    if (rk.source) h += '<p class="edb-ressrc">' + esc(rk.source) + '</p>';
    return h + '</section>';
  }
  function snapshot(o) {
    o = o || {};
    ...
    var preset = PRESETS[o.preset] ? o.preset : (o.report_type === 'SLATE' ? 'SLATE' : o.report_type === 'RANKINGS' ? 'RANKINGS' : 'GAME');
    var reportType = o.report_type === 'SLATE' ? 'SLATE' : o.report_type === 'RANKINGS' ? 'RANKINGS' : 'GAME';
    ...
    /* THE RESEARCH TRAVELS WITH THE BRIEF. A game with no posted price used to
       produce a page of empty market boilerplate. The caller hands in the read
       its own board already computed — the projection, the ratings, what is
       still missing — and it is frozen into the snapshot like everything else,
       so a shared or reprinted brief shows the numbers it was published with.
       NOTHING IS DERIVED HERE: this block only carries and renders. */
    var research = researchBlock(o.research);
    var pubCards = (reportType === 'SLATE' ? sl.picks : cards.slice(0, 1)).map(function (c, i) {
      return { rank: i + 1, brief: publisher(c, { preset: preset, research_follows: !!research, event_label: c.game && c.game.away && c.game.home ? c.game.away + ' at ' + c.game.home : (c.game && c.game.matchup) || null, when_label: c.game && c.game.commence ? whenLabel(c.game.commence, o.tz) : null, sport_label: o.sport_label || (c.game && c.game.sport_label) || null }) };
    });
    ...
    return {
      ...
      public: {
        ...
        research: research,
        rankings: rankingsBlock(o.rankings),
  ...
  function publicPayload(snap) {
    ...
    return {
      ...
      cards: snap.public.cards, watch: snap.public.watch, slate: snap.public.slate,
      research: snap.public.research, rankings: snap.public.rankings, data_status: snap.public.data_status
    };
  }
  ...
  function briefHTML(snap, opts) {
    ...
    } else if (snap.report_type === 'RANKINGS') {
      h += rankingsHTML(pub.rankings);
    } else {
      /* THE RESEARCH LEADS THE PAGE. A game brief that opened with the
         market call put the one thing EdgeDesk might not have at the top and
         buried everything it did have underneath it — so a game no book had
         posted read as an empty page. The order is now the order a reader
         needs: what EdgeDesk makes of the football first, what a sportsbook
         makes of it second. */
      if (pub.research) h += researchHTML(pub.research);
      cards.forEach(function (pc) {
        var b = pc.brief;
        /* A CARD WITH NO PRICE IS SIX HEADINGS EACH SAYING "there is no
           price": price limit not set, no comparison price, no market read,
           nothing to judge. It filled a page and told a writer nothing. When
           the brief carries research instead, the market half collapses to
           the one sentence that is actually news — and it collapses BELOW
           the research rather than in place of it. */
        if (pub.research && !(b.call && b.call.odds)) {
    ...
    h += '<footer class="edb-ft">' + (snap.report_type === 'RANKINGS' ? '' : '<div class="edb-h">EdgeDesk data check</div><p><b>' + esc(ds.status || 'Current') + '</b>'
      ...
      + (snap.report_type !== 'RANKINGS' && ds.prices && ds.prices.length > 1 ? '<ul class="edb-prices">' + ds.prices.map(function (p) { return '<li>' + esc(p.selection || '') + (p.odds ? ' ' + esc(p.odds) : '') + (p.book ? ' · ' + esc(p.book) : '') + (p.captured_at ? ' · captured ' + esc(fmtStamp(p.captured_at)) : '') + '</li>'; }).join('') + '</ul>' : '')
  ...
  /* The same document again with no classes on it, for a CMS paste. Every
     section the styled page carries is here too — a writer who pastes the
     brief into a CMS gets the research, not a market stub. */
  function researchCms(res) {
    if (!res) return '';
    var h = '<h2>The EdgeDesk research</h2>';
    if (res.game) h += '<p><strong>' + esc(res.game.title) + '</strong>' + (res.game.meta ? '<br>' + esc(res.game.meta) : '') + '</p>';
    if (res.lede.length) h += res.lede.map(function (s) { return '<p>' + esc(s) + '</p>'; }).join('');
    else if (res.headline) h += '<p>' + esc(res.headline) + '</p>';
    var p = res.projection;
    if (p && p.priced) {
      h += '<h3>EdgeDesk projection</h3><p><strong>Fair spread: ' + esc(p.fair_spread_text || '') + '</strong></p>';
      ...
      if (p.win_prob) h += '<p>Win probability: ' + esc(p.win_prob.home) + ' ' + esc(p.win_prob.home_text || (p.win_prob.home_pct + '%')) + ', ' + esc(p.win_prob.away) + ' ' + esc(p.win_prob.away_text || (p.win_prob.away_pct + '%')) + '.'
        + (p.win_prob.bounded ? ' ' + esc(p.win_prob.bounded_note || '') : '') + '</p>';
      ...
      h += '<h3>EdgeDesk projection</h3><p>EdgeDesk has not priced this matchup yet'
        + (p.absent_reason ? ' — ' + esc(p.absent_reason) : '') + '.' + (p.note ? ' ' + esc(p.note) : '') + '</p>';
    } else if (res.score) {
    ...
    function cmsTable(cols, rowsOf) {
      var t = '<table><thead><tr><th></th><th>' + esc(cols[0]) + '</th><th>' + esc(cols[1]) + '</th></tr></thead><tbody>';
      t += rowsOf();
      return t + '</tbody></table>';
    }
    if (res.compare) {
      h += '<h3>Team research head-to-head</h3>';
      if (res.compare.one_sided_note) h += '<p>' + esc(res.compare.one_sided_note) + '</p>';
      h += cmsTable(res.compare.cols, function () {
        var t = '';
        res.compare.groups.forEach(function (g) {
          t += '<tr><th colspan="3">' + esc(g.title) + '</th></tr>';
          g.rows.forEach(function (x) {
            function c(y) { return y.v ? (esc(y.v) + (y.rank ? ' ' + esc(y.rank) : '')) : (y.note ? esc('unavailable — ' + y.note) : 'not measured'); }
            t += '<tr><th>' + esc(x.k) + '</th><td>' + c(x.a) + '</td><td>' + c(x.h) + '</td></tr>';
          });
        });
        return t;
      });
    } else if (res.table) {
      h += cmsTable(res.table.cols, function () {
        return res.table.rows.map(function (x) {
          return '<tr><th>' + esc(x.k) + '</th><td>' + esc(x.a || 'not measured') + '</td><td>' + esc(x.h || 'not measured') + '</td></tr>';
        }).join('');
      });
    }
    if (res.advantages && (res.advantages.away.length || res.advantages.home.length)) {
      h += '<h3>Where each team has the edge</h3>';
      [[res.compare ? res.compare.cols[0] : 'Away', res.advantages.away],
       [res.compare ? res.compare.cols[1] : 'Home', res.advantages.home]].forEach(function (pair) {
        h += '<h4>' + esc(pair[0]) + ' advantages</h4>'
          + (pair[1].length ? '<ul>' + pair[1].map(function (a) { return '<li>' + esc(a.k) + ': ' + esc(a.text) + '</li>'; }).join('') + '</ul>'
            : '<p>No category where both teams carry a national rank favours ' + esc(pair[0]) + '.</p>');
      });
    } else if (res.advantages && (res.advantages.measured.length || res.advantages.unproven.length)) {
      h += '<h3>Where each team has the edge</h3><p>' + esc(res.advantages.note || '') + '</p>';
      if (res.advantages.measured.length) h += '<h4>What EdgeDesk can measure</h4>'
        + (res.advantages.measured_note ? '<p>' + esc(res.advantages.measured_note) + '</p>' : '')
        + '<ul>' + res.advantages.measured.map(function (a) { return '<li>' + esc(a.k) + ': ' + esc(a.text) + '</li>'; }).join('') + '</ul>';
      if (res.advantages.unproven.length) h += '<h4>Why the size of the gap is not established</h4><ul>'
        + res.advantages.unproven.map(function (n) { return '<li>' + esc(n) + '</li>'; }).join('') + '</ul>';
    }
    if (res.matchups.length) {
      h += '<h3>The matchups that matter</h3>'
    ...
    if (res.roster) {
      ...
      h += cmsTable(res.roster.cols, function () {
        return res.roster.rows.map(function (x) {
          return '<tr><th>' + esc(x.k) + '</th><td>' + esc(x.a || 'not measured') + '</td><td>' + esc(x.h || 'not measured') + '</td></tr>';
        }).join('');
      });
      if (res.roster.note) h += '<p><em>' + esc(res.roster.note) + '</em></p>';
    }
    if (res.cases) {
      h += '<h3>The case for each team</h3>';
      [res.cases.favourite, res.cases.underdog].forEach(function (c) {
        if (!c) return;
        h += '<h4>' + esc(c.heading) + '</h4><ul>' + c.bullets.map(function (b) { return '<li>' + esc(b) + '</li>'; }).join('') + '</ul>';
      });
    }
    if (res.uncertainty) {
      h += '<h3>Why the number could be wrong</h3><ul>'
        + res.uncertainty.items.map(function (i) { return '<li>[' + esc(i.sev) + '] ' + esc(i.label) + ' — ' + esc(i.text) + '</li>'; }).join('') + '</ul>';
      if (res.uncertainty.unmeasured.length) h += '<h4>Not measured at all</h4><ul>'
        + res.uncertainty.unmeasured.map(function (u) { return '<li>' + esc(u.item) + (u.why ? ' — ' + esc(u.why) : '') + '</li>'; }).join('') + '</ul>';
    }
    if (res.notes.length && !res.projection) h += '<ul>' + res.notes.map(function (n) { return '<li>' + esc(n) + '</li>'; }).join('') + '</ul>';
    var mk = res.market;
    if (mk) {
    ...
    if (res.state) h += '<h3>Research state</h3><p><strong>' + esc(res.state.label) + '</strong>'
      + (res.state.note ? ' — ' + esc(res.state.note) : '') + '</p>'
      + (res.state.scope ? '<p>' + esc(res.state.scope) + '</p>' : '');
    if (res.missing.length) h += '<h3>What EdgeDesk could not measure</h3><ul>' + res.missing.map(function (n) { return '<li>' + esc(n) + '</li>'; }).join('') + '</ul>';
    if (res.source) h += '<p><em>' + esc(res.source) + '</em></p>';
    return h;
  }
  function rankingsCms(rk) {
    if (!rk) return '';
    var h = '';
    if (rk.headline) h += '<p>' + esc(rk.headline) + '</p>';
    if (rk.subhead) h += '<p>' + esc(rk.subhead) + '</p>';
    if (rk.rows.length) {
      h += '<table><thead><tr>' + rk.columns.map(function (c) { return '<th>' + esc(c) + '</th>'; }).join('') + '</tr></thead><tbody>';
      rk.rows.forEach(function (r) {
        h += '<tr><td>' + esc(r.rank || '') + '</td><td>' + esc(r.team) + (r.conf ? ' (' + esc(r.conf) + ')' : '') + '</td>'
          + r.cells.map(function (c) { return '<td>' + esc(c.v || '—') + (c.rank ? ' ' + esc(c.rank) : '') + '</td>'; }).join('')
          + '<td>' + esc(r.delta || '') + '</td></tr>';
      });
      h += '</tbody></table>';
    }
    function secs(list, heading) {
      if (!list.length) return '';
      var out = heading ? '<h2>' + esc(heading) + '</h2>' : '';
      list.forEach(function (x) { out += (x.h ? '<h3>' + esc(x.h) + '</h3>' : '') + (x.p ? '<p>' + esc(x.p) + '</p>' : ''); });
      return out;
    }
    h += secs(rk.method, 'How EdgeDesk ranks teams') + secs(rk.movers, 'What moved this week') + secs(rk.spotlights, null);
    if (rk.missing.length) h += '<h2>What is missing, and why</h2><ul>' + rk.missing.map(function (n) { return '<li>' + esc(n) + '</li>'; }).join('') + '</ul>';
    if (rk.footer.length) h += rk.footer.map(function (n) { return '<p>' + esc(n) + '</p>'; }).join('');
    if (rk.source) h += '<p><em>' + esc(rk.source) + '</em></p>';
    return h;
  }
  ...
  function briefCmsHTML(snap) {
    ...
    if (snap.report_type === 'SLATE') {
      ...
      h += rankingsCms(pub.rankings);
    } else {
      if (pub.research) h += researchCms(pub.research);
      (pub.cards || []).forEach(function (pc) {
        if (pub.research && !(pc.brief.call && pc.brief.call.odds)) {
          h += '<h2>EdgeDesk’s market call</h2><p>' + esc(noVerdictSentence(pub.research)) + '</p>';
        } else h += pick(pc, false);
      });
    }
    if (snap.report_type === 'SLATE' || snap.report_type === 'RANKINGS') h += researchCms(pub.research);
    if (snap.report_type !== 'RANKINGS') h += '<h3>EdgeDesk data check</h3><p>' + esc(ds.status || 'Current') + (ds.price_captured_at ? ' · Price captured at ' + esc(fmtStamp(ds.price_captured_at)) : '') + '</p>';
    h += '<p><em>' + (snap.report_type === 'RANKINGS'
      ? 'EdgeDesk rates teams from its own play-by-play and roster data. It is independent of the AP and Coaches polls and does not vote in either.'
      : 'EdgeDesk judges the price on offer against the rest of the betting market. It does not predict who wins.') + ' 21+. Gamble responsibly — 1-800-GAMBLER.</em></p>';
    h += '<p><em>Powered by EdgeDesk Sports</em></p>';
    return h;
  }
  function pad(v, n) { var t = String(v == null ? '' : v); return t.length >= n ? t + '  ' : t + new Array(n - t.length + 1).join(' '); }
  ...
  function briefText(snap, opts) {
    ...
    if (snap.report_type === 'SLATE') {
      ...
      if (pub.research) researchText();
      (pub.cards || []).forEach(function (pc) {
        /* the same collapse the page makes: a card with no price is six
           headings each saying there is no price, and the research above has
           already been read. */
        var b = pc.brief;
        if (pub.research && !(b.call && b.call.odds)) {
          L.push('EDGEDESK’S MARKET CALL');
          L.push(noVerdictSentence(pub.research));
          L.push('');
        } else pick(pc, false);
      });
    }
    if (snap.report_type === 'RANKINGS') {
      var rk = pub.rankings;
      if (rk) {
        if (rk.headline) { L.push(rk.headline); L.push(''); }
        if (rk.subhead) { L.push(rk.subhead); L.push(''); }
        rk.rows.forEach(function (r) {
          L.push([r.rank, r.team + (r.conf ? ' (' + r.conf + ')' : '')]
            .concat(r.cells.map(function (c) { return (c.v || '—') + (c.rank ? ' ' + c.rank : ''); }))
            .concat(r.delta ? [r.delta] : []).join('  '));
        });
        function tsec(list, heading) {
          if (!list.length) return;
          L.push('');
          if (heading) { L.push(heading.toUpperCase()); L.push(''); }
          list.forEach(function (x) { if (x.h) L.push(x.h.toUpperCase()); if (x.p) L.push(x.p); L.push(''); });
        }
        tsec(rk.method, 'How EdgeDesk ranks teams');
        tsec(rk.movers, 'What moved this week');
        tsec(rk.spotlights, null);
        if (rk.missing.length) { L.push('WHAT IS MISSING, AND WHY'); rk.missing.forEach(function (n) { L.push('  · ' + n); }); L.push(''); }
        rk.footer.forEach(function (n) { L.push(n); });
        if (rk.source) L.push(rk.source);
        L.push('');
      }
    }
    /* THE RESEARCH IS ITS OWN DOCUMENT, and on a game brief it leads.
       Emitted as a function so the plain-text order can match the page's:
       the football first, the market second. */
    function researchText() {
      var res = pub.research;
      if (!res) return;
      L.push('THE EDGEDESK RESEARCH');
      if (res.game) { L.push(res.game.title); if (res.game.meta) L.push(res.game.meta); }
      L.push('');
      if (res.lede.length) res.lede.forEach(function (s) { L.push(s); });
      else if (res.headline) L.push(res.headline);
      var rp = res.projection;
      if (rp && rp.priced) {
        L.push(''); L.push('EDGEDESK PROJECTION');
        ...
        if (rp.win_prob) L.push('  Win probability: ' + rp.win_prob.home + ' ' + (rp.win_prob.home_text || (rp.win_prob.home_pct + '%')) + ', ' + rp.win_prob.away + ' ' + (rp.win_prob.away_text || (rp.win_prob.away_pct + '%'))
          + (rp.win_prob.bounded ? ' (bounded — a projection cannot mean 100% or 0%)' : ''));
        ...
        L.push(''); L.push('EDGEDESK PROJECTION');
        L.push('  EdgeDesk has not priced this matchup yet' + (rp.absent_reason ? ' — ' + rp.absent_reason : '') + '.');
      ...
      if (res.compare) {
        ...
        if (res.compare.one_sided_note) L.push(res.compare.one_sided_note);
        res.compare.groups.forEach(function (g) {
          L.push(''); L.push('  ' + g.title.toUpperCase());
          textTable(res.compare.cols, g.rows.map(function (x) {
            function c(y) { return y.v ? (y.v + (y.rank ? ' ' + y.rank : '')) : (y.note ? 'unavailable' : 'not measured'); }
            return { k: '  ' + x.k, a: c(x.a), h: c(x.h) };
          }));
        });
      } else if (res.table) {
        textTable(res.table.cols, res.table.rows.map(function (x) {
          return { k: x.k, a: x.a || 'not measured', h: x.h || 'not measured' };
        }));
      }
      if (res.advantages && (res.advantages.away.length || res.advantages.home.length)) {
        L.push(''); L.push('WHERE EACH TEAM HAS THE EDGE');
        [[res.compare ? res.compare.cols[0] : 'Away', res.advantages.away],
         [res.compare ? res.compare.cols[1] : 'Home', res.advantages.home]].forEach(function (pair) {
          L.push(''); L.push('  ' + String(pair[0]).toUpperCase() + ' ADVANTAGES');
          if (!pair[1].length) L.push('    none: no category where both teams carry a national rank favours them');
          pair[1].forEach(function (a) { L.push('    · ' + a.k + ': ' + a.text); });
        });
      } else if (res.advantages && (res.advantages.measured.length || res.advantages.unproven.length)) {
        L.push(''); L.push('WHERE EACH TEAM HAS THE EDGE');
        if (res.advantages.note) L.push('  ' + res.advantages.note);
        if (res.advantages.measured.length) {
          L.push(''); L.push('  WHAT EDGEDESK CAN MEASURE');
          if (res.advantages.measured_note) L.push('  ' + res.advantages.measured_note);
          res.advantages.measured.forEach(function (a) { L.push('    · ' + a.k + ': ' + a.text); });
        }
        if (res.advantages.unproven.length) {
          L.push(''); L.push('  WHY THE SIZE OF THE GAP IS NOT ESTABLISHED');
          res.advantages.unproven.forEach(function (n) { L.push('    · ' + n); });
        }
      }
      if (res.matchups.length) {
        L.push(''); L.push('THE MATCHUPS THAT MATTER');
      ...
      if (res.roster) {
        ...
        textTable(res.roster.cols, res.roster.rows.map(function (x) {
          return { k: x.k, a: x.a || 'not measured', h: x.h || 'not measured' };
        }));
        if (res.roster.note) L.push(res.roster.note);
      }
      if (res.cases) {
        L.push(''); L.push('THE CASE FOR EACH TEAM');
        [res.cases.favourite, res.cases.underdog].forEach(function (c) {
          if (!c) return;
          L.push(''); L.push('  ' + c.heading.toUpperCase());
          c.bullets.forEach(function (b) { L.push('    · ' + b); });
        });
      }
      if (res.uncertainty) {
        L.push(''); L.push('WHY THE NUMBER COULD BE WRONG');
        res.uncertainty.items.forEach(function (i) {
          L.push('  [' + i.sev + '] ' + i.label + (i.widening ? ' (widens the range ' + i.widening + ')' : '') + ' — ' + i.text);
        });
        if (res.uncertainty.unmeasured.length) {
          L.push(''); L.push('  NOT MEASURED AT ALL');
          res.uncertainty.unmeasured.forEach(function (u) { L.push('    · ' + u.item + (u.why ? ' — ' + u.why : '')); });
        }
      }
      if (res.notes.length && !res.projection) { L.push(''); res.notes.forEach(function (n, i) { L.push((i + 1) + '. ' + n); }); }
      var rmk = res.market;
      if (rmk) {
      ...
      if (res.state) { L.push(''); L.push('RESEARCH STATE'); L.push('  ' + res.state.label + (res.state.note ? ' — ' + res.state.note : ''));
        if (res.state.scope) L.push('  ' + res.state.scope); }
      if (res.missing.length) { L.push(''); L.push('WHAT EDGEDESK COULD NOT MEASURE'); res.missing.forEach(function (n) { L.push('  · ' + n); }); }
      if (res.source) { L.push(''); L.push(res.source); }
      L.push('');
    
    }
    if (snap.report_type === 'SLATE' || snap.report_type === 'RANKINGS') researchText();
    if (snap.report_type !== 'RANKINGS') {
    ...
    L.push(''); L.push(snap.report_type === 'RANKINGS'
      ? 'EdgeDesk rates teams from its own play-by-play and roster data. It is independent of the AP and Coaches polls and does not vote in either. 21+. Gamble responsibly — 1-800-GAMBLER.'
      : 'EdgeDesk judges the price on offer against the rest of the betting market. It does not predict who wins. 21+. Gamble responsibly — 1-800-GAMBLER.');
  ...
  return {
    ...
    researchBlock: researchBlock, rankingsBlock: rankingsBlock, researchHTML: researchHTML, rankingsHTML: rankingsHTML,
