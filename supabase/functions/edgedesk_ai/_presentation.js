...
(function (root, factory) {
  ...
  function publisher(simple, ctx) {
    ...
    if (!whyLines.length) whyLines.push(v === 'PASS' ? 'EdgeDesk does not see enough value at the current price.'
      ...
      : ctx.research_follows ? 'There is no number to favor yet — no book has posted this game. EdgeDesk’s research on the matchup is below.'
    ...
    if (s.suppressed) lede = 'EdgeDesk is withholding a call on this game until a problem with its own data is fixed.';
      ...
      : (ctx.research_follows
        ? 'No sportsbook has posted a price on this game yet, so EdgeDesk has no bet to call. What follows is EdgeDesk’s own read on the matchup — the projection, the ratings behind it and how much of it is settled.'
        : 'No sportsbook has posted a price on this game yet, so EdgeDesk has no bet to call. ' + (pl.answer || 'Nothing is published in its place.'));
  ...
  function researchBlock(r) {
    ...
    var out = {
      ...
      state: (r.state && txt(r.state.label)) ? { label: txt(r.state.label), note: txt(r.state.note) } : null,
      score: score,
      table: table,
    ...
    return (out.headline || out.state || out.score || out.table || out.notes.length || out.missing.length) ? out : null;
  }
  ...
  function researchHTML(res) {
    ...
    if (res.headline) h += '<p class="edb-lede">' + esc(res.headline) + '</p>';
    if (res.score) {
    ...
    if (res.state) h += '<p class="edb-resstate"><b>' + esc(res.state.label) + '</b>' + (res.state.note ? ' — ' + esc(res.state.note) : '') + '</p>';
    if (res.table) {
    ...
    if (res.notes.length) h += '<ul class="edb-why">' + res.notes.map(function (n) { return '<li>' + esc(n) + '</li>'; }).join('') + '</ul>';
    ...
    if (snap.report_type === 'SLATE') {
      ...
      cards.forEach(function (pc) {
        ...
        /* A CARD WITH NO PRICE IS SIX HEADINGS EACH SAYING "there is no
           ...
           the one sentence that is actually news. */
        if (pub.research && !(b.call && b.call.odds)) {
          h += '<p class="edb-nomkt">EdgeDesk’s verdicts compare a posted price against its own number. No book has posted this game, so there is no price to compare and no verdict on it — not a lean, not a pass. The moment a price is captured this brief carries the comparison; until then what stands is the research below.</p>';
        } else {
          h += briefCardHTML(pc, { numbered: false, showEvent: false, hideAnswer: true });
        }
      });
    }
    /* The research sits under the call, on every report that carries one. */
    h += researchHTML(pub.research);
  ...
  /* The same two blocks again with no classes on them, for a CMS paste. */
  function researchCms(res) {
    if (!res) return '';
    var h = '<h2>The EdgeDesk research</h2>';
    if (res.headline) h += '<p>' + esc(res.headline) + '</p>';
    if (res.score) h += '<p><strong>Projected score:</strong> ' + esc(res.score.away.team) + ' ' + esc(res.score.away.points)
      + ', ' + esc(res.score.home.team) + ' ' + esc(res.score.home.points) + '.' + (res.score.note ? ' ' + esc(res.score.note) : '') + '</p>';
    if (res.state) h += '<p><strong>' + esc(res.state.label) + '</strong>' + (res.state.note ? ' — ' + esc(res.state.note) : '') + '</p>';
    if (res.table) {
      h += '<table><thead><tr><th></th><th>' + esc(res.table.cols[0]) + '</th><th>' + esc(res.table.cols[1]) + '</th></tr></thead><tbody>';
      res.table.rows.forEach(function (x) {
        h += '<tr><th>' + esc(x.k) + '</th><td>' + esc(x.a || 'not measured') + '</td><td>' + esc(x.h || 'not measured') + '</td></tr>';
      });
      h += '</tbody></table>';
    }
    if (res.notes.length) h += '<ul>' + res.notes.map(function (n) { return '<li>' + esc(n) + '</li>'; }).join('') + '</ul>';
  ...
  function briefText(snap, opts) {
    ...
    if (res) {
      ...
      if (res.headline) L.push(res.headline);
      if (res.score) L.push('Projected score: ' + res.score.away.team + ' ' + res.score.away.points + ', ' + res.score.home.team + ' ' + res.score.home.points + '.' + (res.score.note ? ' ' + res.score.note : ''));
      if (res.state) L.push(res.state.label + (res.state.note ? ' — ' + res.state.note : ''));
      if (res.table) {
        L.push('');
        L.push(pad('', 26) + pad(res.table.cols[0], 20) + res.table.cols[1]);
        res.table.rows.forEach(function (x) { L.push(pad(x.k, 26) + pad(x.a || 'not measured', 20) + (x.h || 'not measured')); });
      }
      if (res.notes.length) { L.push(''); res.notes.forEach(function (n, i) { L.push((i + 1) + '. ' + n); }); }
