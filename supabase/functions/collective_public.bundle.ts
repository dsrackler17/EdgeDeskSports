...
//
// ---------------------------------------------------------------------------
// FIXED. Four defects, all in this file. Three are bug fixes and change no
// product behaviour. The fourth is a switch, defaulted to what this function
// does today, because it decides who can see paid numbers.
//
//   1. /v1/games CARRIED NO sport AND NO week. _shared/reads.ts holds
//      buildGames ONCE so the site and the embed render identical data, and
//      the two bundles had drifted: the collective_embed copy carries both
//      fields and this one did not. That is what collective/index.html is
//      working around where it says "the games feed does not always carry a
//      week, and Wundefined is worse than no week at all" -- the site was
//      patching around a shared builder that had stopped being shared.
//
//   2. THE FUNCTION HAD NO OPINION ABOUT ITS OWN CENTRAL RULE. buildGames
//      mapped EVERY row board_models returned, so whether a model appeared
//      once or three times on a game was decided entirely by a view this
//      file says nothing about -- and movement_n, which the site prints as
//      the +n beside a pick to tell a creator their re-upload WAS received,
//      was passed through only if that view happened to expose it. Where it
//      does not, the marker silently never renders and a creator who
//      re-uploaded is told nothing, which is the bug that started all of
//      this. The rule is stated here now: the graded row is the earliest
//      pre-kickoff live submission, a late row never takes that slot, and
//      movement_n is counted when the collapse sees more than one row and
//      taken from the view when it does not.
//
//      That order is the load-bearing half. Counting alone, against a view
//      that already collapses, would report 1 for every row on earth -- the
//      exact false "no revision ever arrived" the marker exists to prevent.
//
//   3. ?season= EMPTIED THE WHOLE BOARD. "" is not null so it survived ??,
//      Number("") is 0, and 0 is finite -- so the season became 0, the
//      game_detail filter matched nothing, and the response was a valid,
//      empty board with no error anywhere. collective/index.html builds
//      exactly that URL: '&season='+encodeURIComponent(sport.season||'').
//      Survivable only because buildMeta always fills a season in.
//
//   4. THE FREE-TIER HOLE, left switched OFF. isEntitled short-circuits on
//      billing.enabled and returns true for anybody holding a session, so
//      every pre-kickoff number here is free to any account that can
//      complete an email sign-in. collective_embed lists this as its own
//      defect 3 and removed it -- so the embed on a member's site and the
//      Collective's own site currently disagree about the same reader.
//
//      One of them is wrong, but closing it changes who can see the board
//      TODAY, and that is a decision about the product rather than a bug to
//      quietly fix. So both behaviours are here and OPEN_BOARD_UNTIL_BILLING
//      selects between them, defaulted to what this function already does.
//      Pasting this file changes nothing about who sees what; flipping one
//      boolean closes the hole and makes the two functions agree.
//
// Also carried over from collective_embed, because the reasoning is the same
// in both: superseded_at is filtered wherever rows are read. It is inert
// until supabase/migrations/01_supersede.sql is applied -- the column does
// not exist, undefined is falsy, nothing is filtered -- so this file can be
// deployed before or after that migration, in either order.
// ---------------------------------------------------------------------------
...
/* Entitlement: an active subscriber, or any creator (they contributed to the
   ...
   The OPEN_BOARD_UNTIL_BILLING branch is the behaviour this function has
   today: while billing is off, any signed-in account is entitled (contract
   5.2 -- the record is being built in the open; anonymous callers stay
   locked). collective_embed removed exactly this branch as its own defect 3,
   which is why the embed and this function currently disagree about the same
   reader. Set the constant at the top of this file to false to make them
   agree; it is left true so pasting this file is a pure bug fix. */
...
/* The season a request means. "" IS NOT ABSENT: ?season= (which the site
   sends whenever its own season is falsy -- see
   '&season='+encodeURIComponent(sport.season||'') in collective/index.html)
   survives ??, Number("") is 0, and 0 is finite. So the season became 0, the
   game_detail filter matched nothing, and the board came back valid and
   empty with nothing anywhere saying why. Blank now means absent; a
   non-numeric season is still a 422. */
