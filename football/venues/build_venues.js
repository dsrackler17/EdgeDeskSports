...
async function main() {
  ...
  /* AND THE SAME DESCRIPTIONS, SMALL ENOUGH FOR THE PAGE TO LOAD.
     The browser reads the trained venue table straight out of params.js, and
     that table carries no city — so the card in the page said "Acrisure
     Stadium" while the committed slate said "Acrisure Stadium, Pittsburgh,
     PA". Two renderers describing one stadium differently is precisely the
     drift football/matchup/forecast.js exists to end one level down, and
     resolved.json is 600 KB, which is not something to hand a phone.
     So the descriptions ship on their own: 136 entries, about 15 KB, as a
     module both the page and Node can load. Descriptive fields only — no
     coordinate, roof or surface, because those belong to the layer the venue
     coefficients were fitted on. */
  const desc = '/* GENERATED by football/venues/build_venues.js — do not edit by hand.\n'
    + '   Descriptive venue fields ONLY (name, city, IANA zone, venue id) for the keys the trained\n'
    + '   parameter table and football/venues/supplement.json already own. The trained table carries\n'
    + '   no city at all, so without this the page cannot say what town a stadium is in while the\n'
    + '   offline build can — and the two then describe the same game differently.\n'
    ...
    + '});\n';
  fs.writeFileSync(path.join(__dirname, 'descriptions.js'), desc);

  log('[venues] ' + written + ' key(s) written to football/venues/resolved.json, ' + held + ' already known, '
    + refused.length + ' refused; ' + Object.keys(describe).length
    + ' description(s) written to football/venues/descriptions.js for the page');
