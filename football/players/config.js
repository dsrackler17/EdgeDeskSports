...
(function (root, factory) {
  ...
  var OBSERVABILITY = {
    ...
    epa:               { observed: 'elsewhere', used_in_rating: false, since: 2014, source: 'football/fbs_epa \u2014 sportsdataverse/cfbfastR-cfb-data adv_passing (per-game passing EPA from the published cfb_model_artifacts expected-points model)',
      reason: 'CORRECTED. This read "observed: false" on the grounds that the play-attribution table carries no next-score information, which is still true of THAT table and was wrongly generalised to every public file. The successor SportsDataverse repository publishes per-game passing EPA for every FBS quarterback from 2014, and football/fbs_epa ingests it. It is NOT wired into this layer\u2019s ratings: it covers passers only, it is not opponent-adjusted, and football/fbs_epa/epa_contract.js establishes it is not on the scale any coefficient in this repository was fitted against. Success rate, explosive rate and yards per play remain what this layer measures; the EPA is published as research beside them rather than invented into them.' },
