...
function nflRows() {
  ...
  return { rows, error: null, source: path.relative(ROOT, file), games: games.length };
}
...
function tierFrom(table) {
  for (const [name, rule] of [['VALIDATED', RULES.tier.validated], ['LEAN', RULES.tier.lean]]) {
    for (const t of RULES.thresholds) {
      const e = table[String(t)];
      if (e && e.n >= rule.min_n && e.win_pct != null && e.win_pct >= rule.win_pct && e.p_one_sided != null && e.p_one_sided < rule.p_max) {
        return { tier: name, required_edge_points: t, basis: name + ' on the tune window: the model side went ' + e.wins + '-' + (e.n - e.wins - e.pushes) + '-' + e.pushes + ' (' + (e.win_pct * 100).toFixed(2) + '%, p ' + e.p_one_sided + ', n ' + e.n + ') at ' + t + '+ points of disagreement' };
      }
    }
  }
  return { tier: 'RESEARCH', required_edge_points: null, basis: 'no disagreement threshold on the tune window cleared break-even with a usable sample; the cover probabilities are arithmetic on the blend, not a betting edge' };
}
...
function runMarket(o) {
  ...
  for (const Sn of seasons) {
    ...
    /* the model number: for a spread the rating margin; for a total the
       rating has nothing to say, so the tune-window mean total is the
       model — a deliberately weak model, stated as such */
    const tuneTotals = tune.map((r) => num(r.close_total)).filter((x) => x != null);
    const meanTotal = mean(tuneTotals);
    const modelOf = market === 'total' ? (() => meanTotal) : ratingMargin;
    ...
    for (const raw of test) {
      ...
      for (const side of sides) {
        ...
        const cand = {
          ...
          model_version: 'rating_blend_' + Sn, model_version_validated: relInputs.model_version_validated,
...
function build(sport) {
  ...
  if (src.error) {
    return { schema: SCHEMA, sport: key, generated_at: new Date().toISOString(), error: src.error,
      markets: { spread: { market: 'spread', mode: 'RESEARCH_ONLY', mode_basis: src.error, positions: 0 }, total: { market: 'total', mode: 'RESEARCH_ONLY', mode_basis: src.error, positions: 0 } },
  ...
  return {
    ...
    rules: RULES, markets: { spread, total },
...
function print(rep) {
  ...
  ['spread', 'total'].forEach((k) => {
    ...
    if (m.shrinkage) console.log('          ' + m.shrinkage.verdict);
    if (m.counterfactual_lean) console.log('          [counterfactual, tier forced to LEAN, never a recommendation] ' + m.counterfactual_lean.reading
...
module.exports = { build, runMarket, atsTable, tierFrom, drawdown, ols, nflRows, cfbRows, RULES, SCHEMA };
