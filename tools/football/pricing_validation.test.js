...
chk('every candidate arm carries a status, the held-out seasons and its reasons; injury arms read NOT_EVALUATED without the archive', Object.keys(arms).length === 12 && Object.values(arms).every((a) => /VALIDATED|CANDIDATE|REJECTED|NOT_EVALUATED/.test(a.status) && Array.isArray(a.reasons) && (a.status === 'NOT_EVALUATED' ? a.holdout_seasons.length === 0 && /injury archive/.test(a.reasons[0]) : a.holdout_seasons.length >= 5)), Object.keys(arms));
/* the injury archive on: a planted two-point cost per home lineman out */
const injRows = ctxRows.map((r, i) => { const olh = i % 5 === 0 ? 2 : i % 3 === 0 ? 1 : 0, ola = i % 7 === 0 ? 1 : 0; return Object.assign({}, r, { margin: r.margin - 2 * (olh - ola), ctx: Object.assign({}, r.ctx, { injuries_on_file: true, ol_out_home: olh, ol_out_away: ola, qb_out_home: 0, qb_out_away: 0, out_home: olh, out_away: ola, dl_out_home: 0, dl_out_away: 0, db_out_home: 0, db_out_away: 0, wr_out_home: 0, wr_out_away: 0 }) }); });
const injArms = V.featureArms(injRows, 'spread');
chk('with the archive on, a planted offensive-line cost is VALIDATED with a coefficient near minus two', injArms.ol_out_diff.status === 'VALIDATED' && injArms.ol_out_diff.latest_coef < -1.5 && injArms.ol_out_diff.latest_coef > -2.5, injArms.ol_out_diff);
chk('an injury arm with no planted effect is not validated', injArms.db_out_diff.status !== 'VALIDATED', injArms.db_out_diff.status);
