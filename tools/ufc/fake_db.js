#!/usr/bin/env node
/* ===========================================================================
   EdgeDesk UFC — an in-memory stand-in for the PostgREST client, for tests.

   It answers the same five calls tools/ufc/db.js answers (select, selectAll,
   upsert, patch, rpc) with the same PostgREST query grammar the jobs write
   (eq / neq / gt / gte / lt / lte / in, order, limit, offset), keeps the same
   conflict semantics (merge-duplicates writes only the columns present;
   ignore-duplicates skips a hit), and implements the live lock the way the
   SQL does. That is enough to drive the sync and the poller end to end,
   twice, and count rows. It is NOT the database: the SQL file is tested
   against a real PostgreSQL by tools/ufc/ufc_sql.test.js.
   =========================================================================== */
'use strict';
...
function parseQuery(q) {
  ...
  String(q || '').split('&').forEach(part => {
    ...
    else if (k === 'order') { const m = /^([a-z_]+)\.(asc|desc)/.exec(v); if (m) out.order = { col: m[1], dir: m[2] }; else out.order = { col: v.split('.')[0], dir: 'asc' }; }
...
function fakeDb(seed) {
  ...
  const api = {
    ...
    async upsert(schema, rel, rows, onConflict, o) {
      ...
      rows.forEach(r => {
        ...
        if (hit) {
          ...
          if (rel === 'fight_snapshots' || rel === 'market_captures' || rel === 'market_rejections' || rel === 'pipeline_runs') row.id = row.id || t.length + 1;
    ...
    async del(schema, rel, q) { const p = parseQuery(q); const t = tbl(schema, rel); const keep = t.filter(r => !p.filters.every(f => matches(r, f))); t.length = 0; keep.forEach(r => t.push(r)); return []; },
    async rpc(schema, fn, args) {
      stats.requests++;
      const now = Date.parse(nowFn());
      if (fn === 'acquire_live_lock') {
        const ttl = Math.max(10, args.p_ttl_seconds || 120), l = locks[args.p_event_id];
        if (!l || l.owner === args.p_owner || l.expires < now) {
          locks[args.p_event_id] = { owner: args.p_owner, expires: now + ttl * 1000, acquired_at: l && l.owner === args.p_owner ? l.acquired_at : now };
      ...
      if (fn === 'release_live_lock') { const l = locks[args.p_event_id]; if (l && l.owner === args.p_owner) { delete locks[args.p_event_id]; return true; } return false; }
...
module.exports = { fakeDb, parseQuery };
