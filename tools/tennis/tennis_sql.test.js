#!/usr/bin/env node
/* ===========================================================================
   EdgeDesk — supabase/tennis_live_center.sql, run against a real PostgreSQL.
   ...
   tools/tennis/sql/tennis_live_center.test.sql: reads as anon and
   authenticated, every write a client could attempt, the tour-day lock's race
   semantics, the dedup constraints, the shape checks and the cascade.

   Same shape and skip behaviour as tools/ufc/ufc_sql.test.js — without
   PostgreSQL it skips loudly and passes; CI runs it with a database.

   Run: node tools/tennis/tennis_sql.test.js
        EDGD_PG="-h 127.0.0.1 -p 5432 -U postgres" node tools/tennis/tennis_sql.test.js
   =========================================================================== */
'use strict';
...
function skip(why) {
  console.log('SKIP | tennis live center SQL | ' + why);
...
try {
  ...
  for (const f of [SHIM, SCHEMA]) {
    ...
    if (r.status !== 0) {
      console.log('FAIL | tennis live center SQL | ' + path.basename(f) + ' did not apply');
  ...
  if (bad.length) {
    console.log('FAIL | tennis live center SQL | the migration report is not all ok');
    bad.forEach((l) => console.log('     | ' + l));
    throw new Error('report');
  }
  const twice = psql(conn, ['-d', DB, '-v', 'ON_ERROR_STOP=1', '-q', '-f', SCHEMA]);
  if (twice.status !== 0) {
    console.log('FAIL | tennis live center SQL | the file is not idempotent — a second run failed');
  ...
  if (r.status !== 0 || /FAIL:/.test(out)) {
    console.log('FAIL | tennis live center SQL | ' + passed + ' passed before the failure');
    console.error(out.split('\n').filter((l) => /FAIL|ERROR/.test(l)).slice(0, 8).join('\n'));
    throw new Error('suite');
  }
  if (passed < 30) {
    console.log('FAIL | tennis live center SQL | only ' + passed + ' assertions ran — the suite exited early');
    throw new Error('short');
  }
  console.log('PASS | tennis live center SQL | ' + passed + ' assertions against a real PostgreSQL');
} catch (e) {
