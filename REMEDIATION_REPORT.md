...
| — | P1 | `modelKey` folds non-totals markets to h2h → possible fabricated EV if the model emits spreads/props. | Market collapsed in the join key. | `modelKey` (~4142) | Documented exact patch; NOT applied — changes a displayed number and needs server confirmation the model emits non-h2h rows. | — | — | **DEFERRED** |
...
- **`modelKey` market-fold (P1).** Deferred deliberately: it changes a displayed model EV/edge number, and the frontend cannot confirm the model actually emits spreads/prop rows to trigger the mismatch. Exact patch documented (include market in the key, or guard EV to same-market rows). Not altered blindly per rule #2/#6.
