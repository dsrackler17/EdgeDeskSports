...
| `econ.reserve_bps` | `1000` |
| `econ.platform_bps` | `3000` |
| `econ.founder_pool_bps` | `6000` |
| `econ.founder_count` | `6` |
| `econ.referral_bps` | `0` |
...
- **model_wall**: one row per listed model of a listed active creator: creator slug and name and logo/monogram, model name, sport, membership status, record summary, coverage pct, last submission, submitted games count, founding flag. `submitted_games` counts distinct games with a graded-eligible live pick, played or not; it is what lets a surface tell "has not submitted" apart from "submitted, waiting on kickoff", both of which leave the record null.
