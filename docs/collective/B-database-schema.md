...
- **model_wall**: one row per listed model of a listed active creator: creator identity, model identity, membership status, record summary, coverage pct, last submission, `submitted_games` (distinct games with a graded-eligible live pick, played or not), founding flag.
