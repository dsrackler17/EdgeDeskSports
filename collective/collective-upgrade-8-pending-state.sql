-- MODEL COLLECTIVE, UPGRADE 8: say what is actually true before results.
-- Run after upgrade 7. Adds one column to the wall view so the site can tell
-- "has not submitted" apart from "submitted, waiting on kickoff". Safe to run
-- twice.
...
-- The view keeps the grants it already had; create or replace does not drop
-- them. Nothing else changes, so no data is touched and no reindex happens.
