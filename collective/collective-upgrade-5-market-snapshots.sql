-- MODEL COLLECTIVE, UPGRADE 5: immutable market snapshots.
-- Run once in the Supabase SQL editor. Safe to run twice. Adds the market
-- history store, the write path collectors use, the rule that attaches a
-- market to a submission, and the closing-line rule. Changes nothing that
-- already works: grading still reads results.closing_spread, and an existing
-- closing spread is never overwritten.
